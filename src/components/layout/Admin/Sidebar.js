import React from "react";
import { Link } from "react-router-dom";
import { RoutesLink } from "../../../ApiHelper/RoutesLink";

export default function Sidebar({ role }) {
    const [sites, setSites] = React.useState(false)
    const [HRM, setHRM] = React.useState(false)
    const [Masters, setMasters] = React.useState(false)

    const toggleHRM = () => {
        setHRM(!HRM)
    }

    const toggleMAsters = () => {
        setMasters(!Masters)
    }

    const toggleSites = () => {
        setSites(!sites)
    }

    const [attendance, setAttendance] = React.useState(false)
    const [leave, setLeave] = React.useState(false)
    const [regularization, setRegularization] = React.useState(false)
    const [reimbursement, setReimbursement] = React.useState(false)
    const [isActive, setIsActive] = React.useState('')


    const toggleIsActive = (link) => {
        setIsActive(link)
    }

    const toggleAttendance = () => {
        setAttendance(!attendance)
    }

    const toggleLeave = () => {
        setLeave(!leave)
    }

    const toggleRegularization = () => {
        setRegularization(!regularization)
    }

    const toggleReimbursement = () => {
        setReimbursement(!reimbursement)
    }


    return (
        <>
            <aside className="sidebar-wrapper" id="sideWrapper">
                <div className="sidebar-content">
                    <Link className="navbar-brand " id="brand" to="">
                        <img class="main-logo" src="../assets/img/Shreenath-logo.svg" alt="main-logo" />

                    </Link>
                    {role === 'admin' && <div className="sidebar-inner">
                        <nav className="aside-nav">
                            <ul className="sidebar-vertical">
                                <li>
                                    <span className="menu-title"> Menu </span>
                                </li>
                                <li>
                                    <Link to={RoutesLink?.dashboard_route}
                                        onClick={() => toggleIsActive('dashboard')}
                                        className={isActive === 'dashboard' ? "drop-link active" : "drop-link"} role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 463.937 463.937"
                                            style={{ enableBackground: "new 0 0 512 512" }} className="">
                                            <g>
                                                <path
                                                    d="m460.414 216.518-85.6-73.1c.1-.5.2-1 .3-1.6v-95.6c-.1-5.7-4.6-10.3-10.3-10.5h-47.8c-5.5 0-10 4.9-10 10.5v39.6l-68.5-58.4c-3.7-3.2-9.2-3.2-13 0l-221.9 189c-4.3 3.5-4.8 9.8-1.3 14.1s9.8 4.8 14.1 1.3l.2-.2 215.4-183.4 77.1 65.7 46.1 39.2 92.3 78.6c4.2 3.6 10.5 3.1 14.1-1.1 3.5-4.2 3-10.5-1.2-14.1zm-105.4-90-28-23.6v-47.1h28v70.7z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="m416.314 236.718-28.1-24-149.7-127.9c-3.7-3.2-9.2-3.2-13 0l-149.7 127.9-28.1 24c-4.2 3.6-4.7 9.9-1.1 14.1 3.5 4.2 9.7 4.7 13.8 1.2l.1-.1 12.5-10.8v187.5c.1 5.6 4.7 10.2 10.3 10.3h297.3c5.6-.1 10.2-4.6 10.3-10.3v-187.5l12.5 10.8c1.8 1.5 4 2.4 6.4 2.4 2.9 0 5.7-1.3 7.6-3.5 3.7-4.3 3.1-10.6-1.1-14.1zm-144.3 182.1h-80v-108h80v108zm20 0v-117.7c0-5.5-4.1-10.3-9.6-10.3h-100.8c-5.5 0-9.6 4.8-9.6 10.3v117.7h-79v-194.8l139-118.4 139 118.4v194.8h-79z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                            </g>
                                        </svg>
                                        <span> Dashboard </span>
                                    </Link>
                                </li>
                                <li>
                                    <Link className="drop-link btn-menu" onClick={toggleSites} role="button">
                                        <span className="menu-title border-0 p-0 m-0"> Sites </span>
                                        <svg className={sites ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 20 20" fill="none">
                                            <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </Link>
                                    <ul className="sub-menu" style={{ display: sites ? 'block' : 'none' }}>
                                        <li>
                                            <Link to={RoutesLink?.sites_route}
                                                onClick={() => toggleIsActive('sites')}
                                                className={isActive === 'sites' ? "drop-link active" : "drop-link"} role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 64 64"
                                                    style={{ enableBackground: "new 0 0 512 512" }}>
                                                    <g>
                                                        <path
                                                            d="M18.179 39.571A1.002 1.002 0 0 0 19 40h.01a1 1 0 0 0 .822-.445c.893-1.34 1.872-2.702 2.91-4.145C27.081 29.37 32 22.525 32 15a13 13 0 0 0-26 0c0 8.019 4.766 14.466 9.375 20.702.94 1.272 1.913 2.588 2.804 3.87ZM19 4a11.012 11.012 0 0 1 11 11c0 6.881-4.72 13.448-8.883 19.243a206.218 206.218 0 0 0-2.133 3.003c-.659-.917-1.338-1.836-2-2.733C12.565 28.538 8 22.36 8 15A11.012 11.012 0 0 1 19 4Z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                        <path
                                                            d="M27 15a8 8 0 1 0-8 8 8.01 8.01 0 0 0 8-8Zm-14 0a6 6 0 1 1 6 6 6.007 6.007 0 0 1-6-6ZM2 62h60a1 1 0 0 0 .98-1.196l-5-25A1 1 0 0 0 57 35h-4v2h3.18l4.6 23H3.22l4.6-23H13v-2H7a1 1 0 0 0-.98.804l-5 25A1 1 0 0 0 2 62ZM39 33a5 5 0 1 0-5-5 5.006 5.006 0 0 0 5 5Zm0-8a3 3 0 1 1-3 3 3.003 3.003 0 0 1 3-3Z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                        <path
                                                            d="M41 36a4.95 4.95 0 0 0 1.1 3.09 4.987 4.987 0 0 0-3.1 1.94 4.987 4.987 0 0 0-3.1-1.94A4.95 4.95 0 0 0 37 36a5 5 0 0 0-10 0 4.95 4.95 0 0 0 1.1 3.09A5.001 5.001 0 0 0 24 44v10a1 1 0 0 0 1 1h28a1 1 0 0 0 1-1V44a5.001 5.001 0 0 0-4.1-4.91A4.95 4.95 0 0 0 51 36a5 5 0 0 0-10 0Zm-9-3a3 3 0 1 1-3 3 3.003 3.003 0 0 1 3-3Zm6 20H26v-9a3.003 3.003 0 0 1 3-3h6a3.003 3.003 0 0 1 3 3Zm8-20a3 3 0 1 1-3 3 3.003 3.003 0 0 1 3-3Zm3 8a3.003 3.003 0 0 1 3 3v9H40v-9a3.003 3.003 0 0 1 3-3Z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                    </g>
                                                </svg>
                                                <span>Sites</span>
                                            </Link>
                                        </li>
                                        {/* <li>

                                        </li> */}

                                        <li className="d-none">
                                            <Link className="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 512 512"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M245.501 358.159c28.361 0 53.841-17.936 63.404-44.63a7.5 7.5 0 0 0-14.121-5.059c-7.433 20.748-27.238 34.688-49.283 34.688-28.861 0-52.342-23.48-52.342-52.342s23.48-52.342 52.342-52.342c23.958 0 44.815 16.192 50.721 39.376a7.5 7.5 0 1 0 14.537-3.703c-7.601-29.835-34.436-50.673-65.257-50.673-37.132 0-67.342 30.209-67.342 67.342s30.209 67.343 67.341 67.343z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M245.501 252.942c-20.884 0-37.875 16.991-37.875 37.875s16.991 37.875 37.875 37.875 37.875-16.991 37.875-37.875-16.991-37.875-37.875-37.875zm0 60.75c-12.613 0-22.875-10.262-22.875-22.875s10.262-22.875 22.875-22.875 22.875 10.262 22.875 22.875-10.262 22.875-22.875 22.875z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="m453.992 283.8-22.739-.391-.222-.002c-6.885 0-12.61 5.538-12.808 12.446-1.301 45.25-20.023 87.573-52.717 119.172-32.451 31.363-75.018 48.586-120.002 48.586-.449 0-.902-.001-1.352-.005-92.413-.709-168.419-75.35-171.355-167.253 2.038.541 4.176.832 6.382.832h.102l4.402 20.396c2.208 10.226 11.403 17.648 21.864 17.648h18.875c9.231 0 17.438-5.743 20.78-14.1h2.649a101.878 101.878 0 0 0 7.164 17.304l-20.023 20.023c-4.86 4.859-5.377 12.622-1.202 18.058a141.631 141.631 0 0 0 26.014 26.014c5.434 4.174 13.196 3.658 18.057-1.202l20.023-20.023a101.993 101.993 0 0 0 17.304 7.164v28.329c0 6.875 5.123 12.729 11.916 13.617a142.321 142.321 0 0 0 18.397 1.199c6.132 0 12.322-.403 18.396-1.199 6.793-.889 11.917-6.743 11.917-13.617v-28.329a101.878 101.878 0 0 0 17.304-7.164l20.023 20.023c4.859 4.86 12.622 5.376 18.058 1.202a141.564 141.564 0 0 0 26.013-26.014c4.176-5.435 3.659-13.198-1.201-18.057l-20.023-20.024a101.946 101.946 0 0 0 7.164-17.304h28.329c6.874 0 12.728-5.123 13.617-11.917.795-6.073 1.198-12.262 1.198-18.397s-.403-12.324-1.198-18.397c-.89-6.793-6.744-11.916-13.617-11.916h-28.329a101.854 101.854 0 0 0-7.164-17.303l20.023-20.024c4.86-4.86 5.376-12.623 1.202-18.057a141.574 141.574 0 0 0-26.014-26.015c-5.435-4.174-13.198-3.658-18.057 1.202l-20.023 20.023a101.946 101.946 0 0 0-17.304-7.164v-28.329c0-6.873-5.123-12.728-11.917-13.617a142.512 142.512 0 0 0-36.795 0c-6.793.89-11.916 6.744-11.916 13.617v28.329a101.85 101.85 0 0 0-17.304 7.164l-15.29-15.289c19.773-16.719 32.447-39.683 36.079-64.942a174.373 174.373 0 0 1 26.828-2.076h146.084v9.559c0 4.003 2.192 7.664 5.72 9.554s7.791 1.687 11.123-.53l50.732-33.752c3.027-2.014 4.834-5.388 4.834-9.024s-1.807-7.01-4.835-9.024l-50.732-33.753a10.822 10.822 0 0 0-11.123-.529 10.82 10.82 0 0 0-5.719 9.553v9.559H247.349a222.85 222.85 0 0 0-32.818 2.444c-5.139-15.653-13.971-29.966-26.063-41.882-20.171-19.879-46.893-30.59-75.23-30.185-27.046.395-52.645 11.181-72.08 30.373-19.439 19.195-30.543 44.655-31.267 71.692-.89 33.237 14.215 65.207 40.425 85.651-16.964 31.455-25.98 66.589-25.997 102.979-.029 59.012 22.99 114.557 64.817 156.403C130.962 488.955 186.493 512 245.501 512c50.783 0 100.427-17.723 139.788-49.905a7.5 7.5 0 1 0-9.495-11.613C339.106 480.479 292.833 497 245.501 497c-55 0-106.764-21.484-145.756-60.494-38.994-39.011-60.454-90.788-60.426-145.792.016-31.686 7.388-62.347 21.273-90.16a36.606 36.606 0 0 1 1.671 4.276l3.659 11.579c-6.99 4.415-11.646 12.205-11.646 21.066 0 6.669 2.64 12.73 6.924 17.205-.033.169-.069.336-.102.505-4.224 4.464-6.822 10.482-6.822 17.098 0 4.665 1.292 9.033 3.533 12.769a189.052 189.052 0 0 0-.096 7.23c.379 49.411 19.963 95.998 55.146 131.18 35.182 35.182 81.769 54.766 131.179 55.146.492.003.979.005 1.47.005 48.891-.001 95.16-18.719 130.424-52.801 35.007-33.834 55.273-78.991 57.212-127.367l18.39.316c-1.809 47.219-20.015 92.698-51.402 128.282a7.5 7.5 0 0 0 11.249 9.923c34.192-38.764 53.803-88.478 55.22-139.985a12.735 12.735 0 0 0-3.553-9.217 12.75 12.75 0 0 0-9.056-3.964zM69.275 237.475c0-5.461 4.443-9.904 9.904-9.904h71.25c5.461 0 9.904 4.443 9.904 9.904s-4.443 9.904-9.904 9.904h-71.25c-5.461 0-9.904-4.443-9.904-9.904zm45.528-115.401c-9.442 0-17.124-7.682-17.124-17.124s7.682-17.124 17.124-17.124 17.124 7.682 17.124 17.124-7.681 17.124-17.124 17.124zm45.529 150.209c0 5.461-4.443 9.904-9.904 9.904h-71.25c-5.461 0-9.904-4.443-9.904-9.904s4.443-9.904 9.904-9.904h71.25c5.461 0 9.904 4.443 9.904 9.904zm-35.91 47.947h-18.875c-3.446 0-6.475-2.445-7.202-5.813l-3.719-17.231h40.578l-3.568 17.175c-.707 3.401-3.741 5.869-7.214 5.869zm79.763-106.261a86.85 86.85 0 0 1 16.191-6.703c5.868-1.759 9.811-7.042 9.811-13.147V165.95a127.46 127.46 0 0 1 30.627 0v28.169c0 6.105 3.943 11.388 9.81 13.147a86.858 86.858 0 0 1 16.191 6.702c5.395 2.906 11.92 1.958 16.236-2.358l19.91-19.91a126.556 126.556 0 0 1 21.657 21.657l-19.91 19.91c-4.315 4.315-5.264 10.839-2.359 16.235a86.822 86.822 0 0 1 6.703 16.191c1.759 5.868 7.042 9.811 13.147 9.811h28.169c.616 5.064.928 10.209.928 15.313s-.312 10.25-.928 15.313h-28.169c-6.105 0-11.388 3.943-13.147 9.81a86.868 86.868 0 0 1-6.703 16.193c-2.904 5.395-1.956 11.919 2.359 16.234l19.91 19.911a126.658 126.658 0 0 1-21.656 21.657l-19.911-19.911c-4.315-4.315-10.84-5.264-16.235-2.359a86.854 86.854 0 0 1-16.192 6.703c-5.868 1.76-9.81 7.043-9.81 13.147v28.169a127.358 127.358 0 0 1-30.627 0v-28.169c0-6.104-3.942-11.387-9.81-13.147a86.891 86.891 0 0 1-16.193-6.703c-5.394-2.904-11.919-1.956-16.235 2.359l-19.91 19.91a126.58 126.58 0 0 1-21.656-21.656l19.91-19.911c4.315-4.315 5.264-10.839 2.359-16.235a86.854 86.854 0 0 1-6.703-16.192c-1.76-5.868-7.043-9.81-13.147-9.81h-.137l1.859-8.947c13.688-.052 24.808-11.202 24.808-24.901 0-6.768-2.717-12.911-7.114-17.404 4.397-4.493 7.114-10.636 7.114-17.404 0-8.861-4.656-16.651-11.646-21.066l3.733-11.813c1.014-3.208 2.529-6.243 4.478-9.04l16.053 16.053c4.317 4.318 10.841 5.266 16.235 2.361zm43.164-129.334H396.86c5.363 0 9.726-4.363 9.726-9.726v-7.066l39.059 25.986-39.059 25.986v-7.067c0-5.363-4.363-9.726-9.726-9.726H245.501c-8.689 0-17.286.598-25.749 1.757-.01-6.127-.538-12.17-1.567-18.08a208.14 208.14 0 0 1 29.164-2.064zM24.885 102.477c.62-23.146 10.142-44.959 26.812-61.42 16.668-16.459 38.602-25.71 61.759-26.048 24.291-.325 47.195 8.833 64.483 25.871 17.292 17.042 26.815 39.795 26.815 64.071 0 27.843-12.599 53.671-34.565 70.861-8.174 6.397-14.078 14.788-17.073 24.265l-3.948 12.495h-26.865v-44.057c24.847-2.882 45.84-19.936 53.528-44.254a7.5 7.5 0 0 0-4.89-9.412 7.498 7.498 0 0 0-9.412 4.89c-5.704 18.04-20.974 30.855-39.226 33.639v-17.197c14.106-3.388 24.624-16.1 24.624-31.231 0-17.713-14.411-32.124-32.124-32.124S82.679 87.237 82.679 104.95c0 15.13 10.518 27.842 24.624 31.23v17.196c-23.471-3.621-41.5-23.957-41.5-48.426 0-27.019 21.982-49.001 49.001-49.001a48.945 48.945 0 0 1 46.459 33.432 7.5 7.5 0 1 0 14.223-4.765 63.93 63.93 0 0 0-60.683-43.667c-35.29 0-64.001 28.711-64.001 64.001 0 32.751 24.734 59.819 56.5 63.546v44.074H80.439l-3.874-12.26c-3.082-9.752-8.833-18.086-16.633-24.1-22.713-17.511-35.814-45.075-35.047-73.733z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="m500.401 200.921-23.983-41.54a13.099 13.099 0 0 0-11.314-6.533h-47.967a13.101 13.101 0 0 0-11.314 6.532l-23.983 41.54a13.098 13.098 0 0 0 0 13.065l23.983 41.541a13.102 13.102 0 0 0 11.314 6.532h47.967c4.653 0 8.989-2.503 11.314-6.533l23.983-41.54a13.096 13.096 0 0 0 0-13.064zm-36.415 46.138h-45.732l-22.866-39.605 22.866-39.605h45.732l22.866 39.605z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M441.12 178.991c-15.694 0-28.462 12.768-28.462 28.462s12.768 28.462 28.462 28.462 28.462-12.768 28.462-28.462-12.768-28.462-28.462-28.462zm0 41.924c-7.423 0-13.462-6.039-13.462-13.462s6.039-13.462 13.462-13.462 13.462 6.039 13.462 13.462-6.039 13.462-13.462 13.462z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Operations</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Plant Status</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Manage Invertors</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Daily Checklist</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Generations</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>

                                        <li className="d-none">
                                            <Link className="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 512 512"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M499.007 294.117c-10.655-18.439-34.318-24.775-52.748-14.126l-114.705 66.197c.382-1.026.72-2.08 1.011-3.161 4.974-18.57-6.085-37.732-24.654-42.717L194.33 269.864c-10.257-3.292-22.818-1.986-32.073 3.353l-78.742 45.45-6.421-11.121c-6.36-10.979-20.472-14.759-31.458-8.417l-25.994 15.023c-10.995 6.348-14.775 20.459-8.428 31.459L85 473.424c3.072 5.324 8.047 9.133 14.008 10.726 1.989.532 4.006.794 6.007.794 3.99 0 7.92-1.045 11.447-3.089l26.013-15.018c5.311-3.056 9.115-8.021 10.711-13.979 1.593-5.946.788-12.144-2.271-17.46l-6.419-11.117 10.946-6.32c5.09-2.942 7.077-2.879 14.748-1.167l113.278 30.343a47.348 47.348 0 0 0 12.273 1.625c8.236 0 16.393-2.171 23.767-6.421l165.342-95.469c8.912-5.14 15.297-13.472 17.978-23.46s1.324-20.393-3.819-29.294zM137.714 448.714c-.486 1.817-1.633 3.323-3.238 4.247l-26.033 15.03c-1.603.929-3.485 1.171-5.301.686a6.96 6.96 0 0 1-4.271-3.259L25.084 337.604c-1.934-3.352-.783-7.652 2.566-9.585l25.994-15.023a6.963 6.963 0 0 1 3.489-.936 7.04 7.04 0 0 1 6.096 3.502l73.809 127.832c.924 1.607 1.164 3.496.676 5.319zm349.644-129.455c-1.571 5.856-5.304 10.735-10.514 13.739l-165.339 95.468c-7.336 4.227-15.822 5.365-23.892 3.199l-113.442-30.387a9.005 9.005 0 0 0-.324-.08c-9.323-2.084-16.199-3.011-26.417 2.893l-10.944 6.318-44.963-77.875 78.738-45.448c5.381-3.105 13.315-3.903 19.292-1.937.142.046.284.089.428.127l113.78 30.5c10.042 2.696 16.023 13.06 13.336 23.094-2.651 9.846-12.607 15.465-22.67 12.796l-.044-.009-85.511-22.641a8.008 8.008 0 0 0-4.1 15.482l85.861 22.733.027.005 13.302 3.54c10.086 2.662 15.804.716 28.979-6.901l121.325-70.018c10.79-6.231 24.637-2.522 30.872 8.27 3.001 5.194 3.789 11.277 2.218 17.13zM88.541 160.06l13.536 3.081a84.112 84.112 0 0 0 4.335 10.457l-7.387 11.739c-4.763 7.541-4.327 15.926 1.108 21.362l11.707 11.707c5.435 5.435 13.818 5.871 21.346 1.119l11.779-7.399a84.145 84.145 0 0 0 10.459 4.336l3.076 13.536c1.944 8.693 8.172 14.31 15.865 14.31h16.582c7.771 0 13.857-5.464 15.888-14.283l3.083-13.562a83.691 83.691 0 0 0 4.854-1.83v40.546c0 10.621 8.653 19.261 19.29 19.261h100.205c10.636 0 19.289-8.641 19.289-19.261v-10.98c1.045.068 2.106.109 3.189.109h92.892c15.758 0 27.392-7.225 33.646-20.895 9.106-19.903 3.729-47.207-.286-61.711-5.305-19.161-14.163-37.973-24.357-52.118 2.974-3.806 4.731-9.267 4.731-15.555 0-5.871-1.636-13.486-6.148-17.81 9.812-15.479 14.496-34.227 7.041-43.484-3.296-4.093-11.227-9.526-27.3.083l-8.455 5.062c-.978.581-1.65.911-2.081 1.097a3.494 3.494 0 0 0-.055-.031l-12.267-6.903c-4.085-2.276-7.572-3.284-10.935-3.129-3.341-.143-6.912.889-10.922 3.144l-12.226 6.877-.076.043c-.426-.184-1.09-.511-2.053-1.086l-8.483-5.079c-16.102-9.614-24.034-4.16-27.33-.055-7.469 9.307-2.735 28.097 7.142 43.596-.093.087-.191.165-.282.255-3.786 3.757-5.958 10.106-5.958 17.419 0 5.764 1.441 10.8 3.912 14.515-4.053 5.571-7.891 11.909-11.399 18.832l-35.988-36.401c-4.107-4.15-9.557-6.436-15.345-6.435-5.788 0-11.237 2.286-15.346 6.438l-9.021 9.125c-.298-.644-.586-1.294-.9-1.93l7.392-11.746c4.763-7.541 4.328-15.926-1.108-21.362l-11.707-11.707c-5.435-5.436-13.82-5.87-21.352-1.115l-11.768 7.406a87.638 87.638 0 0 0-10.434-4.33l-3.087-13.562c-2.019-8.821-8.105-14.3-15.886-14.3h-16.582c-7.702 0-13.931 5.632-15.859 14.309l-3.084 13.552a86.761 86.761 0 0 0-10.443 4.33l-11.777-7.398c-7.542-4.763-15.927-4.326-21.361 1.108l-11.707 11.707c-5.437 5.436-5.871 13.821-1.119 21.346l7.399 11.779a84.178 84.178 0 0 0-4.337 10.461l-13.496 3.072c-8.717 1.938-14.349 8.166-14.349 15.868v16.583c0 7.78 5.479 13.867 14.309 15.887zm207.275 98.364h-23.301v-34.625c0-6.416 5.232-11.636 11.664-11.636s11.637 5.22 11.637 11.636zm64.267-159.006c-.419-.764-1.082-2.529-1.082-5.389 0-1.93.271-3.409.572-4.435 28.037 5.711 56.647 5.689 87.212-.066.309 1.121.571 2.66.571 4.501 0 2.957-.714 4.721-1.131 5.429-26.498 5.932-55.452 5.922-86.143-.04zm-5.686-55.856c1.098.208 3.249.879 6.797 2.998l8.482 5.078c8.275 4.943 13.115 4.137 18.24 1.233l12.186-6.855c1.455-.818 2.227-1.037 2.465-1.09l.604.062.629-.064c.23.049.986.258 2.481 1.091l11.785 6.629.356.203c5.113 2.924 9.954 3.745 18.292-1.215l8.472-5.071c3.534-2.113 5.681-2.788 6.779-2.999.777 4.656-3.093 19.063-13.445 31.148-24.609 4.092-47.842 4.146-70.538.161-10.468-12.13-14.378-26.636-13.586-31.309zm6.962 72.376c14.865 2.647 29.379 3.979 43.458 3.979 13.372 0 26.349-1.209 38.869-3.595 20.376 26.407 35.873 76.737 25.033 100.429-3.603 7.875-9.666 11.542-19.082 11.542h-92.892a31.52 31.52 0 0 1-3.189-.173v-30.413c8.781-.07 16.325-5.13 19.717-13.242 3.418-8.175 1.67-17.177-4.568-23.498l-21.269-21.513c4.164-9.002 8.897-16.995 13.922-23.515zm-81.154-13.697c1.075-1.086 2.481-1.685 3.959-1.685s2.886.599 3.959 1.683l69.188 69.982c2.359 2.391 1.616 5.039 1.187 6.065-.429 1.024-1.784 3.405-5.124 3.405h-7.824a8.008 8.008 0 0 0-8.008 8.008v65.48c0 1.759-1.499 3.246-3.273 3.246h-22.437V223.8c0-15.248-12.404-27.652-27.652-27.652S256.5 208.552 256.5 223.8v34.625h-22.437c-1.774 0-3.274-1.486-3.274-3.246v-65.48a8.008 8.008 0 0 0-8.008-8.008h-7.823c-3.34 0-4.695-2.381-5.124-3.405a6.083 6.083 0 0 1-.421-1.62c-.145-1.252.082-2.899 1.602-4.44l22.489-22.747.017-.017 46.686-47.221zm-97.534 71.428c-20.844 0-37.801-16.944-37.801-37.772s16.957-37.8 37.801-37.8 37.771 16.957 37.771 37.8c0 1.398-.092 2.829-.256 4.272l-20.567 20.803c-3.115 3.157-5.11 6.984-5.899 11.044a37.62 37.62 0 0 1-11.049 1.653zm-92.424-45.627c.31-.196.906-.483 1.848-.692l18.057-4.11a8.007 8.007 0 0 0 5.946-5.692 68.118 68.118 0 0 1 6.519-15.726 8.005 8.005 0 0 0-.176-8.225l-9.875-15.72c-.509-.806-.726-1.431-.806-1.789l11.102-11.101c.358.081.987.299 1.805.816l15.704 9.864a8.007 8.007 0 0 0 8.198.192 70.65 70.65 0 0 1 15.784-6.543 8.01 8.01 0 0 0 5.66-5.937l4.119-18.097c.202-.912.487-1.5.684-1.808h15.696c.199.312.492.913.706 1.848l4.11 18.057a8.01 8.01 0 0 0 5.66 5.937 71.51 71.51 0 0 1 15.802 6.552 8.01 8.01 0 0 0 8.188-.204l15.685-9.871c.806-.509 1.432-.726 1.789-.806l11.101 11.101c-.08.358-.298.986-.812 1.799l-9.864 15.675a8.007 8.007 0 0 0-.187 8.218 69.307 69.307 0 0 1 4.901 10.669l-12.274 12.415c-5.102-24.4-26.767-42.783-52.646-42.783-29.675 0-53.816 24.142-53.816 53.815s24.142 53.788 53.816 53.788c4.83 0 9.559-.631 14.137-1.878 3.657 5.782 9.724 9.4 16.693 9.844a68.076 68.076 0 0 1-12.48 4.791 8.005 8.005 0 0 0-5.692 5.948l-4.105 18.063c-.215.933-.508 1.53-.706 1.839h-15.706c-.195-.306-.48-.894-.688-1.817l-4.11-18.085a8.007 8.007 0 0 0-5.692-5.948 68.11 68.11 0 0 1-15.725-6.519 8.012 8.012 0 0 0-8.226.176l-15.721 9.875c-.805.508-1.431.726-1.788.805l-11.101-11.101c.08-.358.298-.986.812-1.799l9.864-15.676a8.008 8.008 0 0 0 .18-8.23 68.143 68.143 0 0 1-6.519-15.726 8.009 8.009 0 0 0-5.946-5.692l-18.048-4.108c-.941-.215-1.544-.509-1.856-.708v-15.697zm288.51 31.751c0-9.095 6.769-16.62 16.413-19.334v-2.492c0-4.423 3.585-8.008 8.008-8.008s8.008 3.585 8.008 8.008v2.535c6.923 1.995 12.567 6.507 15.056 12.461a8.008 8.008 0 0 1-14.776 6.178c-.652-1.559-3.654-3.755-8.288-3.755-5.053 0-8.405 2.653-8.405 4.408 0 1.906 0 2.776 9.827 4.55 8.037 1.451 22.998 4.151 22.998 20.31 0 9.095-6.769 16.62-16.412 19.335v2.492c0 4.423-3.585 8.008-8.008 8.008s-8.008-3.585-8.008-8.008v-2.51c-6.097-1.732-11.174-5.421-14.057-10.473a8.008 8.008 0 0 1 13.912-7.935c.992 1.739 4.111 3.5 8.152 3.5 5.053 0 8.404-2.653 8.404-4.408 0-1.906 0-2.775-9.827-4.549-8.037-1.451-22.998-4.151-22.998-20.311zM254.94 164.3a8.008 8.008 0 0 1 8.008-8.008h42.69c4.423 0 8.008 3.585 8.008 8.008s-3.585 8.008-8.008 8.008h-42.69a8.008 8.008 0 0 1-8.008-8.008z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                    </g>
                                                </svg>
                                                <span>Assets</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Maintenance</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Manage Assets</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li className="d-none">
                                            <Link className="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 60 60"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M46 15c-1.433 0-14 .112-14 4v24c0 3.888 12.567 4 14 4s14-.112 14-4V19c0-3.888-12.567-4-14-4Zm0 2c6.93 0 11.229 1.233 11.945 2-.716.767-5.015 2-11.945 2s-11.229-1.233-11.945-2c.716-.767 5.015-2 11.945-2Zm12 9.923C57.515 27.681 53.169 29 46 29s-11.515-1.319-12-2.077v-5.652C37.607 22.937 44.927 23 46 23s8.393-.063 12-1.729Zm0 8C57.515 35.681 53.169 37 46 37s-11.515-1.319-12-2.077v-5.652C37.607 30.937 44.927 31 46 31s8.393-.063 12-1.729ZM46 45c-7.14 0-11.485-1.309-12-2.068v-5.661C37.607 38.937 44.927 39 46 39s8.393-.063 12-1.729v5.661C57.485 43.691 53.14 45 46 45ZM7 17.414A2.015 2.015 0 0 0 8.414 18H12a2 2 0 0 0 2-2v-1.139l6.3 2.684a3.028 3.028 0 0 0 1.59 3.932 2.981 2.981 0 0 0 1.172.241 2.948 2.948 0 0 0 1.123-.223l3.936-1.589a2.969 2.969 0 0 0 1.657-3.893l-1.589-3.936a3 3 0 0 0-5.541-.055L14 8.339V2a2 2 0 0 0-2-2H2a2 2 0 0 0-2 2v7.586A2.015 2.015 0 0 0 .586 11Zm16.049-3.047A1 1 0 0 0 24.43 13.3a.953.953 0 0 1 .621-1.029.971.971 0 0 1 1.281.55l1.592 3.943a.971.971 0 0 1-.548 1.286l-3.947 1.593a.98.98 0 0 1-.759-.007.968.968 0 0 1-.527-.555.948.948 0 0 1 .313-1.143 1 1 0 0 0-.194-1.731L14 12.688v-2.175ZM7 14.586 3.414 11H7ZM12 2v14H9v-5a2 2 0 0 0-2-2H2V2ZM7 38.414A2.015 2.015 0 0 0 8.414 39H12a2 2 0 0 0 2-2v-4h7a3 3 0 0 0 5.12 2.117l3-2.995a2.973 2.973 0 0 0 0-4.239l-3-3a3.052 3.052 0 0 0-4.234-.009A2.964 2.964 0 0 0 21 27h-7v-4a2 2 0 0 0-2-2H2a2 2 0 0 0-2 2v7.586A2.015 2.015 0 0 0 .586 32ZM22.28 29a1 1 0 0 0 .856-1.518.959.959 0 0 1 .166-1.189.994.994 0 0 1 1.4 0l3 3a.972.972 0 0 1 0 1.4l-3 3a1 1 0 0 1-1.412 0 .956.956 0 0 1-.156-1.18A1 1 0 0 0 22.28 31H14v-2ZM7 35.586 3.414 32H7ZM12 23v14H9v-5a2 2 0 0 0-2-2H2v-7ZM7 59.414A2.015 2.015 0 0 0 8.414 60H12a2 2 0 0 0 2-2v-6.339l8.646-3.683a3 3 0 0 0 5.542-.051l1.587-3.937a2.968 2.968 0 0 0-1.651-3.9l-3.934-1.582a3 3 0 0 0-3.89 3.947L14 45.139V44a2 2 0 0 0-2-2H2a2 2 0 0 0-2 2v7.586A2.015 2.015 0 0 0 .586 53Zm15.262-15.621a1 1 0 0 0 .194-1.73 1 1 0 0 1 .216-1.7.987.987 0 0 1 .764 0l3.944 1.591a.971.971 0 0 1 .542 1.288l-1.591 3.944a.974.974 0 0 1-1.3.54.948.948 0 0 1-.6-1.023 1 1 0 0 0-1.381-1.067L14 49.487v-2.175ZM7 56.586 3.414 53H7ZM12 44v14H9v-5a2 2 0 0 0-2-2H2v-7Z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M41.1 24.775q-1.024-.1-1.95-.236a1 1 0 0 0-.292 1.979c.647.095 1.333.179 2.05.248a.8.8 0 0 0 .1.005 1 1 0 0 0 .095-2ZM44 25.974a1 1 0 0 0 .982 1.017Q45.486 27 46 27t1.018-.009a1 1 0 0 0-.036-2c-.648.012-1.316.012-1.964 0a1 1 0 0 0-1.018.983ZM51 26.771a.8.8 0 0 0 .1-.005c.717-.069 1.4-.153 2.05-.248a1 1 0 0 0-.292-1.979q-.925.138-1.95.236a1 1 0 0 0 .095 2ZM41.1 32.775q-1.024-.1-1.95-.236a1 1 0 0 0-.292 1.979c.647.095 1.333.179 2.05.248a.8.8 0 0 0 .1.005 1 1 0 0 0 .095-2ZM44 33.974a1 1 0 0 0 .982 1.017Q45.486 35 46 35t1.018-.009a1 1 0 0 0-.018-2h-.018c-.648.012-1.316.012-1.964 0a1.046 1.046 0 0 0-1.018.983ZM51 34.771a.8.8 0 0 0 .1-.005c.717-.069 1.4-.153 2.05-.248a1 1 0 0 0-.292-1.979q-.925.138-1.95.236a1 1 0 0 0 .095 2ZM41.1 40.775q-1.024-.1-1.95-.236a1 1 0 0 0-.292 1.979c.647.095 1.333.179 2.05.248a.8.8 0 0 0 .1.005 1 1 0 0 0 .095-2ZM46.982 40.991c-.648.012-1.316.012-1.964 0a1.026 1.026 0 0 0-1.018.983 1 1 0 0 0 .982 1.017Q45.486 43 46 43t1.018-.009A1 1 0 0 0 48 41.974a1.026 1.026 0 0 0-1.018-.983ZM52.854 40.539q-.925.138-1.95.236a1 1 0 0 0 .095 2 .8.8 0 0 0 .1-.005c.717-.069 1.4-.153 2.05-.248a1 1 0 0 0-.292-1.979Z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Collection</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    {/* check last sub - sub components for drop-inner or drop link */}
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link inner"}
                                                        role="button">
                                                        <span>Daybook</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Collections</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Work Orders</span>
                                                    </Link>
                                                </li>

                                            </ul>
                                        </li>
                                        <li className="d-none">
                                            <Link className="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 512.001 512.001"
                                                    style={{ enableBackground: "new 0 0 512 512" }}>
                                                    <g>
                                                        <path
                                                            d="M341.954 328.358c-12.131 0-22-8.542-22-19.041 0-10.5 9.869-19.042 22-19.042s22 8.542 22 19.042c0 5.522 4.478 10 10 10s10-4.478 10-10c0-18.325-13.654-33.733-32-37.915v-5.201c0-5.522-4.478-10-10-10s-10 4.478-10 10v5.201c-18.346 4.182-32 19.59-32 37.915 0 21.527 18.841 39.041 42 39.041 12.131 0 22 8.542 22 19.042s-9.869 19.042-22 19.042-22-8.542-22-19.042c0-5.522-4.478-10-10-10s-10 4.478-10 10c0 18.325 13.654 33.733 32 37.915v4.32c0 5.522 4.478 10 10 10s10-4.478 10-10v-4.32c18.346-4.182 32-19.59 32-37.915 0-21.528-18.841-39.042-42-39.042zM451.482 268.876a130.294 130.294 0 0 0-53.222-47.101c-4.979-2.394-10.954-.298-13.346 4.68-2.393 4.978-.298 10.952 4.68 13.345C427.505 258.024 452 296.963 452 339.001c0 17.988-4.434 35.827-12.821 51.591-2.594 4.875-.745 10.931 4.131 13.525a10 10 0 0 0 13.525-4.131C466.756 381.342 472 360.253 472 339.001c0-24.96-7.095-49.209-20.518-70.125zM357.673 209.002l-.673-.001c-5.522 0-10 4.477-10 10s4.478 10 10 10l.579.001h.048c5.501 0 9.973-4.446 9.999-9.953.025-5.523-4.431-10.021-9.953-10.047zM294.407 438.202c-37.91-18.225-62.405-57.164-62.405-99.201 0-17.988 4.434-35.827 12.821-51.591 2.594-4.875.745-10.931-4.131-13.525-4.874-2.594-10.932-.745-13.525 4.131-9.921 18.645-15.165 39.733-15.165 60.985 0 24.96 7.095 49.208 20.518 70.125a130.304 130.304 0 0 0 53.222 47.102 10.002 10.002 0 0 0 8.665-18.026zM327.002 449.001l-.579-.002c-5.548-.01-10.02 4.429-10.045 9.953-.026 5.523 4.432 10.021 9.954 10.047l.67.002c5.522 0 10-4.478 10-10s-4.478-10-10-10z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                        <path
                                                            d="M387.409 175.168a45.9 45.9 0 0 0-4.443-8.168H409c25.364 0 46-20.636 46-46s-20.636-46-46-46h-80.08A45.733 45.733 0 0 0 337 49c0-25.364-20.636-46-46-46H46C20.636 3 0 23.636 0 49s20.636 46 46 46h80.08c-5.091 7.402-8.08 16.357-8.08 26s2.988 18.598 8.08 26H100c-25.364 0-46 20.636-46 46 0 14.567 6.814 27.566 17.415 36C60.814 237.434 54 250.433 54 265s6.814 27.566 17.415 36C60.814 309.434 54 322.433 54 337s6.814 27.566 17.415 36C60.814 381.434 54 394.433 54 409c0 25.364 20.636 46 46 46h117.85c31.046 33.207 75.212 54.001 124.151 54.001 93.738 0 170-76.262 170-170 0-78.017-52.83-143.918-124.592-163.833zM409 95c14.337 0 26 11.664 26 26s-11.663 26-26 26H164c-14.337 0-26-11.664-26-26s11.663-26 26-26h245zM46 75c-14.337 0-26-11.664-26-26s11.663-26 26-26h245c14.337 0 26 11.664 26 26s-11.663 26-26 26H46zm54 92h245c4.028 0 7.885.936 11.345 2.612-4.73-.397-9.512-.611-14.344-.611-46.945 0-89.505 19.129-120.299 49.999H100c-14.336 0-26-11.664-26-26s11.664-26 26-26zm0 72h104.615c-11.352 15.553-20.129 33.095-25.703 52H100c-14.337 0-26-11.663-26-26 0-14.336 11.663-26 26-26zm0 72h74.32a170.4 170.4 0 0 0-2.319 28.001c0 8.146.589 16.156 1.702 23.999H100c-14.337 0-26-11.663-26-26s11.663-26 26-26zm0 124c-14.337 0-26-11.663-26-26s11.663-26 26-26h77.786a169.33 169.33 0 0 0 23.993 52H100zm242.001 54.001c-82.71 0-150-67.29-150-150s67.29-150 150-150 150 67.29 150 150-67.29 150-150 150z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                    </g>
                                                </svg>
                                                <span>Accounts</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Daybook</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Vendor Ledgers</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Budget</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Expense Head</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li className="d-none">
                                            <Link className="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 512 512"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M172.55 391.902c-.13-.64-.32-1.27-.57-1.88-.25-.6-.56-1.18-.92-1.72a9.57 9.57 0 0 0-1.24-1.52 9.57 9.57 0 0 0-1.52-1.24 9.87 9.87 0 0 0-1.73-.92c-.6-.25-1.23-.45-1.87-.57a9.847 9.847 0 0 0-3.9 0c-.64.12-1.27.32-1.88.57-.6.25-1.18.56-1.72.92-.55.36-1.06.78-1.52 1.24-.46.46-.88.97-1.24 1.52-.37.54-.67 1.12-.92 1.72a9.69 9.69 0 0 0-.57 1.88c-.13.64-.2 1.3-.2 1.95 0 .65.07 1.31.2 1.95.12.64.32 1.27.57 1.87.25.61.55 1.19.92 1.73.36.55.78 1.06 1.24 1.52.46.46.97.88 1.52 1.24.54.361 1.12.671 1.72.921.61.25 1.24.45 1.88.57.64.13 1.3.2 1.95.2.65 0 1.31-.07 1.95-.2.64-.12 1.27-.32 1.87-.57.61-.25 1.19-.561 1.73-.921a9.57 9.57 0 0 0 1.52-1.24c.46-.46.88-.97 1.24-1.52.36-.54.67-1.12.92-1.73a10.098 10.098 0 0 0 .77-3.82c0-.65-.07-1.31-.2-1.95z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M459.993 394.982a9.705 9.705 0 0 0-.121-.297c-9.204-21.537-30.79-29.497-56.336-20.772l-69.668 19.266c-4.028-12.198-14.075-22.578-28.281-27.85a11.876 11.876 0 0 0-.265-.094l-76.581-25.992c-6.374-8.239-26.34-29.321-63.723-29.321-26.125 0-49.236 17.922-62.458 37.457H10c-5.523 0-10 4.477-10 10v126.077c0 5.523 4.477 10 10 10h59.457c5.523 0 10-4.477 10-10v-8.634h27.883c5.523 0 10-4.477 10-10v-2.878c16.254 1.418 21.6 4.501 36.528 13.109 11.48 6.62 28.831 16.625 60.077 30.674.145.065.292.127.439.185 5.997 2.359 17.72 6.065 32.173 6.065 10.06 0 21.445-1.797 33.131-7.094l153.991-55.136c.274-.098.544-.208.808-.33 14.717-6.771 36.648-25.854 25.506-54.435zM59.457 473.455H20V367.378h39.457v106.077zm37.883-18.634H79.457v-87.443H97.34v87.443zm329.156-23.747-153.922 55.111a12.33 12.33 0 0 0-.854.348c-21.437 9.852-41.814 3.954-49.8.849-30.182-13.581-46.291-22.87-58.061-29.657-16.364-9.436-24.249-13.984-46.519-15.823V361.36c9.479-15.536 27.861-31.439 47.679-31.439 33.986 0 48.387 22.105 48.953 22.997a10 10 0 0 0 5.305 4.232l79.475 26.974c12.693 4.764 19.401 15.634 16.318 26.474a19.354 19.354 0 0 1-9.257 11.691 19.367 19.367 0 0 1-14.683 1.758l-89.593-28.392c-5.268-1.669-10.886 1.247-12.554 6.512-1.669 5.265 1.247 10.885 6.512 12.554l89.749 28.441c.095.03.19.059.286.086a39.657 39.657 0 0 0 10.857 1.523c6.638 0 13.203-1.691 19.161-5.011 9.213-5.133 15.875-13.547 18.759-23.692.23-.81.434-1.62.611-2.43l75.083-20.8c10.844-3.704 25.079-5.039 31.417 9.558 6.56 17.137-10.49 26.564-14.922 28.678zM359.06 131.543c-.13-.64-.32-1.27-.58-1.88-.25-.6-.55-1.18-.92-1.72a9.57 9.57 0 0 0-1.24-1.52 9.57 9.57 0 0 0-1.52-1.24c-.54-.36-1.12-.67-1.72-.92a9.81 9.81 0 0 0-1.87-.57 9.898 9.898 0 0 0-3.91 0c-.64.12-1.27.32-1.87.57-.61.25-1.19.56-1.73.92-.55.36-1.06.78-1.52 1.24-.46.46-.88.97-1.24 1.52-.36.54-.67 1.12-.92 1.72a9.69 9.69 0 0 0-.57 1.88c-.13.64-.2 1.3-.2 1.95 0 .65.07 1.31.2 1.95.12.64.32 1.27.57 1.87.25.61.56 1.19.92 1.73.36.55.78 1.06 1.24 1.52.46.46.97.88 1.52 1.24.54.36 1.12.67 1.73.92.6.25 1.23.44 1.87.57s1.3.2 1.95.2c.65 0 1.31-.07 1.96-.2.63-.13 1.26-.32 1.87-.57.6-.25 1.18-.56 1.72-.92a9.57 9.57 0 0 0 1.52-1.24c.46-.46.88-.97 1.24-1.52.37-.54.67-1.12.92-1.73.26-.6.45-1.23.58-1.87.13-.64.19-1.3.19-1.95 0-.65-.06-1.31-.19-1.95z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M502 33.891h-59.457c-5.523 0-10 4.477-10 10v8.634H404.66c-5.523 0-10 4.477-10 10v2.878c-16.254-1.419-21.6-4.501-36.527-13.109-11.48-6.62-28.831-16.625-60.078-30.674a8.51 8.51 0 0 0-.44-.185c-10.171-4.002-36.828-11.876-65.299 1.027l-40.24 14.408-33.919-33.918c-3.905-3.905-10.237-3.905-14.142 0L32.657 114.309c-3.602 3.603-4.293 9.85 0 14.143l190.287 190.287c3.045 3.046 10.175 3.967 14.143 0l101.665-101.664c2.643.228 5.386.351 8.229.351 26.126 0 49.236-17.922 62.457-37.456H502c5.523 0 10-4.477 10-10V43.891c0-5.523-4.477-10-10-10zm-350.915-9.726 22.792 22.792c-6.775 4.19-14.608 6.432-22.792 6.432-8.185 0-16.017-2.241-22.792-6.432l22.792-22.792zM76.663 144.173 53.871 121.38l22.792-22.792c4.19 6.775 6.432 14.608 6.432 22.792 0 8.184-2.241 16.017-6.432 22.793zm153.353 153.352-22.788-22.788c13.913-8.586 31.661-8.586 45.575 0l-22.787 22.788zm37.195-37.194c-22.098-16.03-52.292-16.03-74.39 0L91.07 158.579c7.809-10.74 12.025-23.641 12.025-37.199 0-13.559-4.215-26.459-12.025-37.199l22.817-22.816c10.74 7.809 23.64 12.025 37.199 12.025 13.559 0 26.459-4.216 37.199-12.025l21.629 21.629a39.465 39.465 0 0 0-13.462 4.592c-7.168 3.994-12.792 9.975-16.294 17.211a56.985 56.985 0 0 0-29.915 15.741c-22.225 22.226-22.225 58.389.001 80.615 11.112 11.112 25.709 16.669 40.307 16.669 14.597 0 29.195-5.556 40.308-16.669 7.23-7.23 12.295-16.116 14.832-25.8l33.764 11.459c-3.801 17.608.092 36.132 10.593 50.682l-22.837 22.837zm-60.798-98.313c.088.032.176.064.265.094l19.996 6.787c-1.51 6.815-4.927 13.081-9.957 18.112-14.428 14.426-37.904 14.428-52.33 0-14.428-14.427-14.428-37.902 0-52.33a36.977 36.977 0 0 1 12.062-8.048c1.846 15.362 12.907 29.055 29.964 35.385zm98.044 61.066a43.739 43.739 0 0 1-6.389-20.796 77.755 77.755 0 0 0 17.636 9.549l-11.247 11.247zm90.202-57.101c-9.478 15.538-27.86 31.441-47.678 31.441-3.708 0-7.183-.264-10.432-.734l-.039-.006c-21.596-3.137-33.213-15.411-37.042-20.271-.204-.3-1.073-1.437-1.202-1.626a9.964 9.964 0 0 0-5.511-4.583l-79.508-26.985c-12.688-4.762-19.395-15.627-16.321-26.463l.006-.021.007-.025a19.355 19.355 0 0 1 9.247-11.656 19.351 19.351 0 0 1 14.683-1.757l89.593 28.391c5.266 1.671 10.886-1.247 12.554-6.512 1.668-5.265-1.247-10.885-6.512-12.554l-71.255-22.58-.622-.622-.019-.019-36.89-36.89 31.708-11.354c.107-.039.239-.088.345-.131l.105-.042a9.59 9.59 0 0 0 .403-.174c21.436-9.852 41.812-3.955 49.799-.849 30.183 13.581 46.293 22.87 58.063 29.657 16.364 9.437 24.249 13.984 46.518 15.823v80.542zm37.884-6.015H414.66V72.525h17.883v87.443zm59.457 0h-39.457V53.891H492v106.077z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Purchase</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Purchase Book</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Purchase Bill</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Items</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Units</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Groups</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li className="d-none">
                                            {/* <Link className="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1" 
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 74 74"
                                                    style={{enableBackground:"new 0 0 512 512"}} className="">
                                                    <g><text fill="red" font-family="ArialMT, Arial" font-size="18"
                                                        transform="translate(31.995 -18.544)" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">4
                                                        <tspan x="-5.005" y="124" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">14</tspan>
                                                        <tspan x="-5.005" y="248" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">24</tspan>
                                                        <tspan x="-5.005" y="372" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">34</tspan>
                                                        <tspan x="-5.005" y="496" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">44</tspan>
                                                        <tspan x="-5.005" y="620" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">54</tspan>
                                                        <tspan x="-5.005" y="744" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">64</tspan>
                                                        <tspan x="-5.005" y="868" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">74</tspan>
                                                        <tspan x="-5.005" y="992" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">84</tspan>
                                                        <tspan x="-5.005" y="1116" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">94</tspan>
                                                        <tspan x="-10.011" y="1240" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">104</tspan>
                                                        <tspan x="-9.343" y="1364" letter-spacing="-.074em"
                                                            svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">1</tspan>
                                                        <tspan x="-.668" y="1364" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">14</tspan>
                                                        <tspan x="-10.011" y="1488" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">124</tspan>
                                                        <tspan x="-10.011" y="1612" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">134</tspan>
                                                        <tspan x="-10.011" y="1736" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">144</tspan>
                                                        <tspan x="-10.011" y="1860" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">154</tspan>
                                                        <tspan x="-10.011" y="1984" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">164</tspan>
                                                        <tspan x="-10.011" y="2108" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">174</tspan>
                                                        <tspan x="-10.011" y="2232" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">184</tspan>
                                                        <tspan x="-10.011" y="2356" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">194</tspan>
                                                        <tspan x="-10.011" y="2480" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">204</tspan>
                                                        <tspan x="-10.011" y="2604" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">214</tspan>
                                                        <tspan x="-10.011" y="2728" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">224</tspan>
                                                        <tspan x="-10.011" y="2852" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">234</tspan>
                                                        <tspan x="-10.011" y="2976" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">244</tspan>
                                                        <tspan x="-10.011" y="3100" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">254</tspan>
                                                        <tspan x="-10.011" y="3224" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">264</tspan>
                                                        <tspan x="-10.011" y="3348" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">274</tspan>
                                                        <tspan x="-10.011" y="3472" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">284</tspan>
                                                        <tspan x="-10.011" y="3596" svgjs:data="{&quot;leading&quot;:&quot;1.3&quot;}">294</tspan>
                                                    </text>
                                                        <g data-name="2 PT">
                                                            <path
                                                                d="M71 22H3a1 1 0 0 1-.908-1.419l6-13A1 1 0 0 1 9 7h56a1 1 0 0 1 .908.581l6 13A1 1 0 0 1 71 22ZM4.563 20h64.874L64.36 9H9.64Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M7.857 34A5.864 5.864 0 0 1 2 28.143V21a1 1 0 0 1 1-1h9.714a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 7.857 34ZM4 22v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M17.571 34a5.864 5.864 0 0 1-5.857-5.857V21a1 1 0 0 1 1-1h9.714a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 17.571 34Zm-3.857-12v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M27.286 34a5.864 5.864 0 0 1-5.857-5.857V21a1 1 0 0 1 1-1h9.714a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 27.286 34Zm-3.857-12v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M37 34a5.864 5.864 0 0 1-5.857-5.857V21a1 1 0 0 1 1-1h9.714a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 37 34Zm-3.857-12v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M46.714 34a5.864 5.864 0 0 1-5.857-5.857V21a1 1 0 0 1 1-1h9.714a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 46.714 34Zm-3.857-12v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M56.429 34a5.864 5.864 0 0 1-5.857-5.857V21a1 1 0 0 1 1-1h9.714a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 56.429 34Zm-3.858-12v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M66.143 34a5.864 5.864 0 0 1-5.857-5.857V21a1 1 0 0 1 1-1H71a1 1 0 0 1 1 1v7.143A5.864 5.864 0 0 1 66.143 34Zm-3.857-12v6.143a3.857 3.857 0 0 0 7.714 0V22Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                            <path d="M68 70H5.91a1 1 0 0 1-1-1V32h2v36H67V32h2v37a1 1 0 0 1-1 1Z" fill="#000000"
                                                                opacity="1" data-original="#000000" class=""></path>
                                                            <path
                                                                d="M60 70H43a1 1 0 0 1-1-1V41a1 1 0 0 1 1-1h17a1 1 0 0 1 1 1v28a1 1 0 0 1-1 1Zm-16-2h15V42H44ZM37 61.5H12a1 1 0 0 1-1-1v-20a1 1 0 0 1 1-1h25a1 1 0 0 1 1 1v20a1 1 0 0 1-1 1Zm-24-2h23v-18H13Z"
                                                                fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                        </g>
                                                    </g>
                                                </svg>
                                                <span>Store</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link> */}
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link"}
                                                        role="button">
                                                        <span>Purchase Book</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link"}
                                                        role="button">
                                                        <span>Purchase Bill</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <span>Items</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link inner"}
                                                        role="button">
                                                        <span>Units</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link inner"}
                                                        role="button">
                                                        <span>Groups</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li className="d-none">
                                            <Link
                                                onClick={() => toggleIsActive('')}
                                                className={isActive === '' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 480 480.584"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M89.691 130.633c22.774 0 41.235-18.461 41.235-41.235 0-22.77-18.461-41.23-41.235-41.23-22.77 0-41.23 18.46-41.23 41.23.027 22.762 18.469 41.207 41.23 41.235zm0-66.465c13.938 0 25.235 11.297 25.235 25.23 0 13.938-11.297 25.235-25.235 25.235-13.933 0-25.23-11.297-25.23-25.235.016-13.925 11.305-25.21 25.23-25.23zm0 0"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="m448.43 355.383-3.09-80.719 15.055-20.703-18.254-33.555 3.664-11.277 34.488-16.418v-33.422l-34.488-16.418-3.664-11.277 18.254-33.555-19.649-27.023L403.196 78l-9.598-6.977-4.961-37.863-31.762-10.328-26.277 27.719h-11.86l-26.277-27.719-31.77 10.328-1.55 11.809-111.114-6.617-7.488-7.489-2.07-16.8-29.985-12.461-13.359 10.437H84.293L70.898 1.602l-30 12.445-2.062 16.8-7.68 7.68-16.8 2.067L1.89 70.609 12.348 84v10.855L1.89 108.191l9.914 23.914L.765 273.641a46.139 46.139 0 0 0-.425 5.437v.297H.293V280a47.932 47.932 0 0 0 67.375 43.879l47.344-19.727c-7.672 21.66 1.191 45.715 21.09 57.215l-73.13 23.563c-24.19 4.824-40.785 27.203-38.378 51.754C27 461.234 47.625 479.96 72.293 480h.422l294.578-15.45 4.512 10.817 25.437 5.219 20.649-20.059 5.257-1.742 28.551 3.68 17.242-19.426-7.05-27.91 1.12-5.434 17.458-22.886-8.176-24.641zM275.234 45.246l12.563-4.078 24.062 25.383h25.602l24.062-25.383 12.57 4.078 4.536 34.684 20.719 15.054 34.398-6.398 7.754 10.68L424.797 130l7.918 24.36 31.578 15.038v13.204l-31.578 15.039L424.797 222l16.703 30.727-7.754 10.68-34.398-6.4-20.72 15.056-4.534 34.68-12.57 4.081-24.063-25.383h-25.602l-24.062 25.383-12.563-4.082-4.543-34.664-20.718-15.062-34.403 6.398-7.758-10.68 16.704-30.718-7.91-24.368-31.579-15.039v-13.203l31.578-15.039 7.91-24.367-16.703-30.719 7.758-10.68L249.973 95l20.718-15.078zM20.293 68.121l5.277-12.8 13.035-1.602 15.454-15.43 1.601-13.031 12.8-5.281 10.345 8.078h21.879L110.98 20l12.801 5.281 1.598 13.024 15.418 15.457 13.031 1.597 5.29 12.801-8.083 10.344v21.879l8.082 10.344-5.289 12.8-13.031 1.602-15.457 15.457-1.602 13.031-12.797 5.285-10.343-8.078h-21.84l-10.348 8.078-12.797-5.285-1.601-13.031-15.442-15.54-13.023-1.597-5.277-12.8 8.078-10.344v-21.84zm-4 212.024.094-1.25C18 262.41 32.586 250.293 49.09 251.735s28.77 15.902 27.496 32.421c-1.27 16.516-15.602 28.93-32.129 27.832-16.531-1.101-29.094-15.304-28.164-31.843zm144 7.855c17.672 0 32 14.328 32 32s-14.328 32-32 32c-17.676 0-32-14.328-32-32s14.324-32 32-32zm-120 144a31.993 31.993 0 0 1 18.543-28.937l7.734-2.489A31.97 31.97 0 0 1 97.7 412.82a31.966 31.966 0 0 1 3.16 33.301A31.967 31.967 0 0 1 72.59 464h-.395c-17.636-.055-31.902-14.363-31.902-32zm69.39 30.023a47.858 47.858 0 0 0 9.872-38.523 47.843 47.843 0 0 0-22.672-32.676l78.137-25.136a47.999 47.999 0 0 0 32.835-36.754A48.006 48.006 0 0 0 191 282.62a48.015 48.015 0 0 0-48.781-7.047l-48.606 20.258c7.02-19.977.094-42.191-17.035-54.64-17.133-12.446-40.398-12.169-57.23.68l8-102.063 3.879.48 7.68 7.68 2.066 16.8 30.007 12.415 13.36-10.399h10.855l13.352 10.399 29.922-12.383 2.07-16.801 7.68-7.68 16.8-2.074 12.418-29.988-10.402-13.356V84l10.403-13.36-6.145-14.847 85.8 5.078-1.327 10.16L246.164 78l-37.543-6.984-19.648 27.023 18.246 33.555-3.656 11.254-34.489 16.441v33.422l34.48 16.418 3.665 11.277-18.246 33.555 19.648 27.023L246.164 274l9.602 6.969 4.957 37.87 31.77 10.329 26.28-27.719h11.856l26.277 27.719 31.77-10.328 4.96-37.863 9.598-6.977 26.305 4.895 1.914 50.058-2.043-4.89-25.445-5.231-20.695 20.047-5.258 1.754-28.551-3.68-17.242 19.422 7.035 27.914-1.121 5.43-17.457 22.883 8.199 24.644 27.687 7.875 4.153 3.68v.054zm338.403-59.128-2.594 12.593 5.863 23.254-6.007 6.762-23.785-3.047-12.192 4.047-17.207 16.695-8.855-1.808-9.235-22.125-9.597-8.54-23.067-6.558-2.863-8.594 14.547-19.062 2.582-12.582-5.864-23.258 6.008-6.77 23.785 3.059 12.2-4.05 17.215-16.704 8.855 1.824 9.242 22.137 9.598 8.52 23.066 6.55 2.856 8.594zm0 0"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M64.293 280c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zM176.293 320c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zM88.293 432c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zM400.293 360c-22.094 0-40 17.91-40 40s17.906 40 40 40c22.09 0 40-17.91 40-40-.027-22.082-17.922-39.973-40-40zm0 64c-13.254 0-24-10.746-24-24s10.746-24 24-24 24 10.746 24 24-10.746 24-24 24zM320.293 256c44.184 0 80-35.816 80-80s-35.816-80-80-80-80 35.816-80 80c.047 44.164 35.836 79.953 80 80zm0-144c35.344 0 64 28.652 64 64s-28.656 64-64 64c-35.348 0-64-28.652-64-64 .039-35.328 28.668-63.96 64-64zm0 0"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M320.293 224c26.508 0 48-21.492 48-48s-21.492-48-48-48c-26.512 0-48 21.492-48 48 .027 26.5 21.5 47.973 48 48zm0-80c17.672 0 32 14.328 32 32s-14.328 32-32 32c-17.676 0-32-14.328-32-32s14.324-32 32-32zM432.293 24c0 13.254 10.746 24 24 24s24-10.746 24-24-10.746-24-24-24-24 10.746-24 24zm32 0a8 8 0 1 1-16 0 8 8 0 0 1 16 0zM224.293 16c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zm0 0"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Machinery</span>

                                                <svg className="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu inner" style={{ display: "none" }}>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link inner"}
                                                        role="button">
                                                        <span>Readings</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to=""
                                                        onClick={() => toggleIsActive('')}
                                                        className={isActive === '' ? "drop-link inner active" : "drop-link inner"}
                                                        role="button">
                                                        <span>Manage</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>
                                        <li>
                                            <Link to={RoutesLink?.vendors_route}
                                                onClick={() => toggleIsActive('vendor')}
                                                className={isActive === 'vendor' ? "drop-link  active" : "drop-link "}

                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="16" height="16" x="0" y="0" viewBox="0 0 512 512"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="m498.52 219.49-52.86-20.46c-3.8-12.69-8.92-25.05-15.28-36.84l22.92-51.85c1.17-2.65.59-5.74-1.45-7.78l-42.41-42.41a7.005 7.005 0 0 0-7.78-1.45l-51.85 22.92c-11.79-6.35-24.15-11.48-36.84-15.28l-20.46-52.86a6.991 6.991 0 0 0-6.53-4.47h-59.95c-2.89 0-5.49 1.78-6.53 4.47l-20.46 52.86c-12.69 3.8-25.05 8.92-36.84 15.28L110.35 58.7a7.002 7.002 0 0 0-7.78 1.45l-42.41 42.41a7.005 7.005 0 0 0-1.45 7.78l22.92 51.85a198.527 198.527 0 0 0-15.3 36.84l-52.84 20.46a6.991 6.991 0 0 0-4.47 6.53v59.95c0 2.89 1.78 5.49 4.47 6.53l52.86 20.46a195.85 195.85 0 0 0 15.28 36.85l-22.92 51.88a6.988 6.988 0 0 0 1.46 7.78l42.41 42.38a7.002 7.002 0 0 0 7.78 1.45l51.85-22.89c11.7 6.31 24.06 11.44 36.85 15.27l20.46 52.84a6.991 6.991 0 0 0 6.53 4.47H286c2.89 0 5.49-1.78 6.53-4.47l20.46-52.84c12.71-3.83 25.08-8.96 36.84-15.3l51.85 22.92c2.65 1.17 5.74.59 7.78-1.45l42.41-42.38a7.008 7.008 0 0 0 1.46-7.78l-22.92-51.88a195.85 195.85 0 0 0 15.28-36.85l52.86-20.46a6.991 6.991 0 0 0 4.47-6.53v-59.95c0-2.89-1.78-5.49-4.47-6.53zm-9.53 61.68-51.62 19.98c-2.06.8-3.62 2.52-4.22 4.65-3.89 13.95-9.5 27.48-16.68 40.23a6.977 6.977 0 0 0-.3 6.26l22.38 50.66-35.62 35.6-50.63-22.38a7.007 7.007 0 0 0-6.26.3c-12.75 7.16-26.29 12.79-40.25 16.71-2.12.6-3.84 2.16-4.63 4.21l-19.98 51.59h-50.36l-19.98-51.59a6.972 6.972 0 0 0-4.64-4.21 182.73 182.73 0 0 1-40.24-16.68 6.976 6.976 0 0 0-6.26-.3l-50.63 22.36-35.62-35.6 22.38-50.66c.89-2.02.78-4.34-.3-6.26-7.18-12.75-12.79-26.28-16.68-40.23a7.002 7.002 0 0 0-4.22-4.65l-51.62-19.98v-50.36l51.6-19.98c2.05-.8 3.62-2.51 4.21-4.63 3.93-13.96 9.55-27.5 16.71-40.25a7.007 7.007 0 0 0 .3-6.26l-22.38-50.63 35.62-35.62 50.63 22.38c2.02.89 4.34.78 6.26-.3 12.78-7.18 26.31-12.79 40.24-16.68a7.002 7.002 0 0 0 4.65-4.22l19.98-51.62h50.36l19.98 51.62c.8 2.06 2.52 3.62 4.65 4.22 13.92 3.89 27.46 9.5 40.24 16.68a7.007 7.007 0 0 0 6.26.3l50.63-22.38 35.62 35.62-22.38 50.63c-.89 2.02-.78 4.34.3 6.26 7.18 12.78 12.79 26.32 16.68 40.24a7.002 7.002 0 0 0 4.22 4.65l51.62 19.98v50.36zM380.26 168.79c-3.3-4.69-6.87-9.19-10.67-13.47a6.95 6.95 0 0 0-1.42-1.59c-.2-.22-.41-.44-.62-.67-.42-.63-.94-1.19-1.54-1.66-27.69-29.11-66.77-47.29-110.03-47.29s-82.34 18.18-110.02 47.31c-.58.46-1.09 1-1.5 1.61-4.58 4.96-8.84 10.23-12.74 15.77-.45.49-.84 1.04-1.14 1.64-16.68 24.38-26.45 53.85-26.45 85.55s9.78 61.18 26.47 85.56c.29.57.66 1.09 1.08 1.56a153.135 153.135 0 0 0 39.57 38.83c.46.4.99.75 1.56 1.03 23.91 15.71 52.48 24.86 83.17 24.86s57.83-8.7 81.37-23.69c.61-.29 1.16-.65 1.65-1.07a153.157 153.157 0 0 0 41.29-39.95c.44-.48.81-1.01 1.11-1.6 16.69-24.38 26.46-53.84 26.46-85.55s-9.77-61.16-26.44-85.53c-.31-.62-.7-1.18-1.16-1.68zm-12.11 44.41c0 7.1-5.77 12.87-12.87 12.87s-12.84-5.77-12.84-12.87v-47.05h18.03c2.7 3.14 5.26 6.4 7.68 9.77v37.29zm-224.3-37.29c2.99-4.17 6.2-8.17 9.62-11.97h16.09v49.27c0 7.1-5.77 12.87-12.87 12.87s-12.84-5.77-12.84-12.87v-37.29zm39.71-9.76h25.71v47.05c0 7.1-5.76 12.87-12.84 12.87s-12.87-5.77-12.87-12.87zm39.71-2.21h25.71v49.27c0 7.1-5.76 12.87-12.84 12.87s-12.87-5.77-12.87-12.87zm39.71 2.21h25.74v47.05c0 7.1-5.77 12.87-12.87 12.87s-12.87-5.77-12.87-12.87zm39.74-2.21h25.71v49.27c0 7.1-5.77 12.87-12.87 12.87s-12.84-5.77-12.84-12.87zm41.24-14H168.01c23.89-19.85 54.56-31.8 87.96-31.8s64.09 11.95 87.98 31.8zm-214.13 50.5v111.13c-7.52-17.01-11.71-35.8-11.71-55.56s4.19-38.56 11.71-55.57zm55.06 173.64c13.61-25.83 40.46-42.21 69.98-42.21s57.11 16.77 70.53 43.2c-20.4 11.94-44.12 18.79-69.42 18.79s-50.31-7.23-71.08-19.78zm41.86-84.36c0-15.51 12.61-28.12 28.12-28.12s28.15 12.61 28.15 28.12-12.63 28.15-28.15 28.15-28.12-12.63-28.12-28.15zm110.41 77.64a92.699 92.699 0 0 0-55.09-45.47c9.14-7.74 14.96-19.28 14.96-32.17 0-23.23-18.91-42.12-42.15-42.12s-42.12 18.9-42.12 42.12c0 12.9 5.82 24.45 14.97 32.19a93.048 93.048 0 0 0-54.47 44.3 139.29 139.29 0 0 1-29.41-30.11v-99.29c3.82 2.09 8.19 3.28 12.84 3.28 7.87 0 14.95-3.4 19.87-8.8 4.92 5.4 12.01 8.8 19.87 8.8s14.93-3.4 19.85-8.8c4.92 5.4 12 8.8 19.87 8.8s14.93-3.4 19.85-8.8c4.92 5.4 12 8.8 19.87 8.8s14.95-3.4 19.87-8.8c4.91 5.4 11.99 8.8 19.85 8.8s14.95-3.4 19.87-8.8c4.91 5.4 11.99 8.8 19.85 8.8 4.66 0 9.04-1.19 12.87-3.29v99.29c-8.6 12-19.07 22.57-30.99 31.27zm44.99-55.79V200.45c7.52 17.01 11.71 35.81 11.71 55.57s-4.19 38.55-11.71 55.56z"
                                                            fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                    </g>
                                                </svg>
                                                <span>Vendors</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to={RoutesLink?.customers_route}
                                                onClick={() => toggleIsActive('customer')}
                                                className={isActive === 'customer' ? "drop-link  active" : "drop-link "}

                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 512 512"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M139.342 476.615c-.001 0-.001 0 0 0zm370.886-134.414c-2.254 5.152-6.367 9.114-11.581 11.154l-29.341 11.48a20.937 20.937 0 0 1-7.633 1.449c-2.866 0-5.73-.595-8.441-1.781a20.871 20.871 0 0 1-9.495-8.274l-33.374 14.23c1.061 6.473.066 13.297-3.293 19.405-3.956 7.193-10.505 12.396-18.439 14.65-.082.023-.165.039-.248.061 2.985 8.031 2.626 17.231-1.777 25.237-5.539 10.07-15.98 15.771-26.735 15.771a30.26 30.26 0 0 1-6.673-.758 30.03 30.03 0 0 1-3.685 15.965c-5.538 10.07-15.979 15.771-26.734 15.771a30.45 30.45 0 0 1-3.925-.26 30.212 30.212 0 0 1-3.666 16.782c-5.538 10.069-15.979 15.771-26.734 15.771a30.267 30.267 0 0 1-14.624-3.764l-10.531-5.791c-3.877 5.422-9.409 9.407-15.923 11.381a30.709 30.709 0 0 1-8.913 1.331 30.18 30.18 0 0 1-14.274-3.596c-6.992-3.739-11.929-9.837-14.361-16.79a31.219 31.219 0 0 1-4.002 1.525 30.702 30.702 0 0 1-8.912 1.331c-4.935 0-9.815-1.21-14.275-3.596-6.327-3.384-10.966-8.701-13.596-14.834a30.824 30.824 0 0 1-6.513 2.829 30.702 30.702 0 0 1-8.912 1.331 30.192 30.192 0 0 1-14.274-3.596c-7.151-3.825-12.392-10.227-14.756-18.029-1.452-4.79-1.704-9.742-.801-14.507-.602.036-1.203.067-1.803.067a30.19 30.19 0 0 1-14.274-3.595c-7.151-3.825-12.392-10.228-14.756-18.03-2.364-7.801-1.56-16.036 2.265-23.187l7.685-14.369c-1.989-4.433-2.879-9.294-3.745-14.037-1.611-8.832-2.776-13.151-6.487-14.592-2.742-1.064-11.015-4.699-26.495-11.645-4.018 5.104-10.169 8.022-16.541 8.022-2.919 0-5.881-.61-8.704-1.901l-28.656-13.096C1.737 332.9-2.919 320.406 1.897 309.867l58.894-128.865c4.816-10.54 17.311-15.195 27.849-10.378l28.656 13.095c10.539 4.818 15.195 17.311 10.379 27.85l-1.783 3.901 25.94 11.967c.194.09.385.188.572.292 10.32 5.809 21.027 3.28 34.578.078 17.665-4.173 39.411-9.299 64.877 5.732 35.022-21.676 61.915-10.711 81.782-2.591 14.444 5.904 23.752 9.287 32.734 4.616L391 222.383l-.549-1.404c-4.222-10.791 1.122-23.005 11.912-27.228l29.342-11.481c10.793-4.22 23.004 1.124 27.228 11.912l.001.001 51.627 131.943c2.039 5.215 1.92 10.923-.333 16.075zm-456.001-8.417 58.893-128.865c1.13-2.473-.004-5.517-2.478-6.647l-28.655-13.095a4.966 4.966 0 0 0-3.83-.124 4.97 4.97 0 0 0-2.815 2.6L16.447 316.518c-1.13 2.473.004 5.516 2.477 6.646L47.58 336.26c2.474 1.131 5.517-.005 6.647-2.476zm72.027 93.719c3.695-1.12 6.723-3.586 8.534-6.946l14.284-26.701c3.688-6.894.98-15.881-5.917-19.554-6.88-3.664-15.853-1.034-19.572 5.92l-14.258 26.658c-3.696 6.911-.97 15.873 5.928 19.562 3.384 1.809 7.292 2.185 11.001 1.061zm40.194 29.076 19.319-36.122c3.682-6.883.973-15.885-5.916-19.554-6.833-3.639-15.803-1.055-19.543 5.87l-19.349 36.171c-1.81 3.382-2.187 7.29-1.062 11.001s3.607 6.751 6.99 8.561c3.383 1.809 7.288 2.186 11 1.062s6.753-3.607 8.561-6.989zm43.296 14.27 15.622-29.212c3.688-6.897.982-15.882-5.916-19.555-6.929-3.69-15.874-.997-19.573 5.92l-15.622 29.212c-3.759 7.028-1.099 15.803 5.929 19.562 3.383 1.809 7.289 2.187 11.001 1.061 3.711-1.124 6.751-3.606 8.559-6.988zm41.55 17.53 13.671-25.563c3.653-6.83.937-15.905-5.916-19.554-6.928-3.689-15.873-.999-19.573 5.92l-13.673 25.566c-3.755 7.027-1.095 15.8 5.931 19.558 3.384 1.81 7.292 2.188 11.001 1.061 3.71-1.124 6.75-3.606 8.559-6.988zm45.277-21.95-13.819-7.6c-.071-.039-.135-.086-.205-.126a30.24 30.24 0 0 1-3.473 11.659l-7.998 14.956 10.463 5.754c6.985 3.841 15.789 1.285 19.629-5.698 3.618-6.577 1.527-14.795-4.597-18.945zm90.675-103.964-77.657-44.262c-13.276-7.569-29.794-.88-48.916 6.862-28.834 11.673-64.72 26.201-95.073-15.626a8 8 0 0 1 2.498-11.64c23.048-13.203 42.417-27.662 57.571-42.975a166.233 166.233 0 0 1 11.741-10.799c-17.068-7.654-31.557-4.236-46.751-.646-14.823 3.501-30.146 7.121-45.81-1.542l-25.61-11.815-45.867 100.363c9.148 4.104 21.991 9.834 25.087 11.036 11.48 4.457 14.333 15.498 16.024 24.402 3.55-3.654 7.991-6.378 13.019-7.902 7.804-2.364 16.037-1.56 23.188 2.265 7.13 3.797 12.37 10.189 14.741 17.99a31.01 31.01 0 0 1 1.166 5.73c7.071-1.544 14.366-.554 20.789 2.881 7.13 3.797 12.37 10.188 14.741 17.99.136.449.26.899.375 1.35.428-.15.858-.294 1.295-.427 7.802-2.364 16.037-1.559 23.188 2.265 7.13 3.798 12.369 10.189 14.741 17.99.137.449.26.9.375 1.351.427-.15.858-.295 1.294-.427 7.799-2.364 16.035-1.56 23.188 2.265a30.103 30.103 0 0 1 8.978 7.287l40.303 22.345c6.98 3.838 15.788 1.286 19.63-5.697 3.804-6.915 1.217-15.738-5.767-19.668l-47.934-26.979a8 8 0 1 1 7.848-13.944l63.315 35.31c6.98 3.839 15.788 1.285 19.63-5.698 3.777-6.867 1.246-15.612-5.63-19.581-.445-.223-56.995-32.138-56.995-32.138a8 8 0 0 1-3.031-10.9 7.997 7.997 0 0 1 10.9-3.031l55.67 31.445c3.255 1.838 7.076 2.274 10.756 1.228 3.791-1.077 6.914-3.553 8.794-6.97 3.782-6.875 1.179-15.708-5.804-19.688zm50.346-21.009-40.721-104.068-23.031 12.327c-16.094 8.371-31.427 2.105-46.255-3.956-20.252-8.278-41.194-16.837-71.461 4.086-6.34 4.382-12.76 9.844-19.082 16.233-14.344 14.494-32.111 28.205-52.902 40.833 21.399 23.209 44.321 13.933 70.529 3.324 21.056-8.524 42.828-17.34 62.844-5.932l77.656 44.262a30.71 30.71 0 0 1 8.627 7.301zm58.067-9.499-51.627-131.943c-.978-2.5-3.999-3.822-6.498-2.843l-29.341 11.48c-2.531.991-3.834 3.967-2.843 6.499l51.628 131.943a4.974 4.974 0 0 0 2.666 2.753 4.965 4.965 0 0 0 3.831.09l29.341-11.48c1.234-.483 2.212-1.43 2.753-2.667s.574-2.598.09-3.832zm-65.283-121.609c-6.138 0-11.145 5.006-11.145 11.144s5.007 11.145 11.145 11.145 11.145-5.007 11.145-11.145-5.006-11.144-11.145-11.144zM88.399 198.733c-6.146-.343-11.405 4.361-11.748 10.507s4.361 11.405 10.507 11.748 11.405-4.361 11.748-10.507-4.361-11.406-10.507-11.748zm1.122-52.576c-3.951-5.36-5.042-11.814-3.072-18.173 5.486-17.715 18.169-31.502 34.285-38.613-6.281-6.409-10.164-15.177-10.164-24.838 0-19.583 15.933-35.516 35.516-35.516S181.6 44.949 181.6 64.532c0 9.657-3.879 18.421-10.154 24.829a61.71 61.71 0 0 1 20.899 15.159 80.628 80.628 0 0 1 34.692-25.634c-9.666-8.22-15.814-20.457-15.814-34.109C211.223 20.087 231.311 0 256.001 0s44.777 20.087 44.777 44.778c0 13.653-6.149 25.89-15.814 34.11a80.63 80.63 0 0 1 34.692 25.635 61.72 61.72 0 0 1 20.899-15.159c-6.275-6.409-10.155-15.173-10.155-24.83 0-19.583 15.933-35.515 35.516-35.515s35.516 15.932 35.516 35.515c0 9.662-3.883 18.43-10.164 24.839 16.115 7.111 28.799 20.897 34.284 38.613 1.97 6.359.879 12.813-3.072 18.173-3.953 5.36-9.795 8.312-16.452 8.312h-69.706v6.7c-.001 15.13-12.311 27.439-27.439 27.439H203.118c-15.13 0-27.438-12.309-27.438-27.439v-6.702h-69.707c-6.66-.001-12.502-2.952-16.452-8.312zm276.387-62.108h.015c10.757-.004 19.509-8.757 19.509-19.515 0-10.761-8.755-19.515-19.516-19.515S346.4 53.773 346.4 64.534c0 10.758 8.751 19.511 19.508 19.515zm-37.305 34.831a80.38 80.38 0 0 1 6.579 19.591h70.846c1.529 0 2.665-.574 3.574-1.806.907-1.23 1.119-2.485.667-3.946-6.05-19.537-23.869-32.664-44.342-32.669h-.019c-14.934.002-28.569 6.975-37.305 18.83zM255.98 73.555h.041c15.859-.011 28.758-12.916 28.758-28.777C284.778 28.91 271.869 16 256.001 16s-28.778 12.91-28.778 28.778c0 15.861 12.899 28.766 28.757 28.777zm-64.3 73.352v14.263c0 6.308 5.131 11.439 11.438 11.439h105.764c6.308 0 11.438-5.132 11.439-11.439v-14.263c-3.709-32.721-31.278-57.342-64.301-57.352H256l-.021-.001c-33.023.011-60.592 24.633-64.299 57.353zm-45.602-62.86h.014c10.758-.004 19.508-8.757 19.508-19.515 0-10.761-8.754-19.516-19.515-19.516s-19.516 8.755-19.516 19.516c0 10.759 8.751 19.511 19.509 19.515zm-43.679 52.617c.908 1.231 2.043 1.805 3.573 1.805h70.847a80.413 80.413 0 0 1 6.578-19.59c-8.736-11.855-22.372-18.828-37.305-18.831h-.019c-20.474.005-38.293 13.132-44.343 32.669-.451 1.46-.239 2.715.669 3.947z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                    </g>
                                                </svg>
                                                <span>Customers</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link
                                                to={RoutesLink?.item_master_route}
                                                onClick={() => toggleIsActive('item-master')}
                                                className={isActive === 'item-master' ? "drop-link  active" : "drop-link "}
                                          
                                                role="button"
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1" 
                                                 width="18" height="18" x="0" y="0" viewBox="0 0 512 512" 
                                                 style={{enableBackground:"new 0 0 512 512"}}
                                                 className="">
                                                 <g>
                                                 <path d="M139.342 476.615c-.001 0-.001 0 0 0zm370.886-134.414c-2.254 5.152-6.367 9.114-11.581 11.154l-29.341 11.48a20.937 20.937 0 0 1-7.633 1.449c-2.866 0-5.73-.595-8.441-1.781a20.871 20.871 0 0 1-9.495-8.274l-33.374 14.23c1.061 6.473.066 13.297-3.293 19.405-3.956 7.193-10.505 12.396-18.439 14.65-.082.023-.165.039-.248.061 2.985 8.031 2.626 17.231-1.777 25.237-5.539 10.07-15.98 15.771-26.735 15.771a30.26 30.26 0 0 1-6.673-.758 30.03 30.03 0 0 1-3.685 15.965c-5.538 10.07-15.979 15.771-26.734 15.771a30.45 30.45 0 0 1-3.925-.26 30.212 30.212 0 0 1-3.666 16.782c-5.538 10.069-15.979 15.771-26.734 15.771a30.267 30.267 0 0 1-14.624-3.764l-10.531-5.791c-3.877 5.422-9.409 9.407-15.923 11.381a30.709 30.709 0 0 1-8.913 1.331 30.18 30.18 0 0 1-14.274-3.596c-6.992-3.739-11.929-9.837-14.361-16.79a31.219 31.219 0 0 1-4.002 1.525 30.702 30.702 0 0 1-8.912 1.331c-4.935 0-9.815-1.21-14.275-3.596-6.327-3.384-10.966-8.701-13.596-14.834a30.824 30.824 0 0 1-6.513 2.829 30.702 30.702 0 0 1-8.912 1.331 30.192 30.192 0 0 1-14.274-3.596c-7.151-3.825-12.392-10.227-14.756-18.029-1.452-4.79-1.704-9.742-.801-14.507-.602.036-1.203.067-1.803.067a30.19 30.19 0 0 1-14.274-3.595c-7.151-3.825-12.392-10.228-14.756-18.03-2.364-7.801-1.56-16.036 2.265-23.187l7.685-14.369c-1.989-4.433-2.879-9.294-3.745-14.037-1.611-8.832-2.776-13.151-6.487-14.592-2.742-1.064-11.015-4.699-26.495-11.645-4.018 5.104-10.169 8.022-16.541 8.022-2.919 0-5.881-.61-8.704-1.901l-28.656-13.096C1.737 332.9-2.919 320.406 1.897 309.867l58.894-128.865c4.816-10.54 17.311-15.195 27.849-10.378l28.656 13.095c10.539 4.818 15.195 17.311 10.379 27.85l-1.783 3.901 25.94 11.967c.194.09.385.188.572.292 10.32 5.809 21.027 3.28 34.578.078 17.665-4.173 39.411-9.299 64.877 5.732 35.022-21.676 61.915-10.711 81.782-2.591 14.444 5.904 23.752 9.287 32.734 4.616L391 222.383l-.549-1.404c-4.222-10.791 1.122-23.005 11.912-27.228l29.342-11.481c10.793-4.22 23.004 1.124 27.228 11.912l.001.001 51.627 131.943c2.039 5.215 1.92 10.923-.333 16.075zm-456.001-8.417 58.893-128.865c1.13-2.473-.004-5.517-2.478-6.647l-28.655-13.095a4.966 4.966 0 0 0-3.83-.124 4.97 4.97 0 0 0-2.815 2.6L16.447 316.518c-1.13 2.473.004 5.516 2.477 6.646L47.58 336.26c2.474 1.131 5.517-.005 6.647-2.476zm72.027 93.719c3.695-1.12 6.723-3.586 8.534-6.946l14.284-26.701c3.688-6.894.98-15.881-5.917-19.554-6.88-3.664-15.853-1.034-19.572 5.92l-14.258 26.658c-3.696 6.911-.97 15.873 5.928 19.562 3.384 1.809 7.292 2.185 11.001 1.061zm40.194 29.076 19.319-36.122c3.682-6.883.973-15.885-5.916-19.554-6.833-3.639-15.803-1.055-19.543 5.87l-19.349 36.171c-1.81 3.382-2.187 7.29-1.062 11.001s3.607 6.751 6.99 8.561c3.383 1.809 7.288 2.186 11 1.062s6.753-3.607 8.561-6.989zm43.296 14.27 15.622-29.212c3.688-6.897.982-15.882-5.916-19.555-6.929-3.69-15.874-.997-19.573 5.92l-15.622 29.212c-3.759 7.028-1.099 15.803 5.929 19.562 3.383 1.809 7.289 2.187 11.001 1.061 3.711-1.124 6.751-3.606 8.559-6.988zm41.55 17.53 13.671-25.563c3.653-6.83.937-15.905-5.916-19.554-6.928-3.689-15.873-.999-19.573 5.92l-13.673 25.566c-3.755 7.027-1.095 15.8 5.931 19.558 3.384 1.81 7.292 2.188 11.001 1.061 3.71-1.124 6.75-3.606 8.559-6.988zm45.277-21.95-13.819-7.6c-.071-.039-.135-.086-.205-.126a30.24 30.24 0 0 1-3.473 11.659l-7.998 14.956 10.463 5.754c6.985 3.841 15.789 1.285 19.629-5.698 3.618-6.577 1.527-14.795-4.597-18.945zm90.675-103.964-77.657-44.262c-13.276-7.569-29.794-.88-48.916 6.862-28.834 11.673-64.72 26.201-95.073-15.626a8 8 0 0 1 2.498-11.64c23.048-13.203 42.417-27.662 57.571-42.975a166.233 166.233 0 0 1 11.741-10.799c-17.068-7.654-31.557-4.236-46.751-.646-14.823 3.501-30.146 7.121-45.81-1.542l-25.61-11.815-45.867 100.363c9.148 4.104 21.991 9.834 25.087 11.036 11.48 4.457 14.333 15.498 16.024 24.402 3.55-3.654 7.991-6.378 13.019-7.902 7.804-2.364 16.037-1.56 23.188 2.265 7.13 3.797 12.37 10.189 14.741 17.99a31.01 31.01 0 0 1 1.166 5.73c7.071-1.544 14.366-.554 20.789 2.881 7.13 3.797 12.37 10.188 14.741 17.99.136.449.26.899.375 1.35.428-.15.858-.294 1.295-.427 7.802-2.364 16.037-1.559 23.188 2.265 7.13 3.798 12.369 10.189 14.741 17.99.137.449.26.9.375 1.351.427-.15.858-.295 1.294-.427 7.799-2.364 16.035-1.56 23.188 2.265a30.103 30.103 0 0 1 8.978 7.287l40.303 22.345c6.98 3.838 15.788 1.286 19.63-5.697 3.804-6.915 1.217-15.738-5.767-19.668l-47.934-26.979a8 8 0 1 1 7.848-13.944l63.315 35.31c6.98 3.839 15.788 1.285 19.63-5.698 3.777-6.867 1.246-15.612-5.63-19.581-.445-.223-56.995-32.138-56.995-32.138a8 8 0 0 1-3.031-10.9 7.997 7.997 0 0 1 10.9-3.031l55.67 31.445c3.255 1.838 7.076 2.274 10.756 1.228 3.791-1.077 6.914-3.553 8.794-6.97 3.782-6.875 1.179-15.708-5.804-19.688zm50.346-21.009-40.721-104.068-23.031 12.327c-16.094 8.371-31.427 2.105-46.255-3.956-20.252-8.278-41.194-16.837-71.461 4.086-6.34 4.382-12.76 9.844-19.082 16.233-14.344 14.494-32.111 28.205-52.902 40.833 21.399 23.209 44.321 13.933 70.529 3.324 21.056-8.524 42.828-17.34 62.844-5.932l77.656 44.262a30.71 30.71 0 0 1 8.627 7.301zm58.067-9.499-51.627-131.943c-.978-2.5-3.999-3.822-6.498-2.843l-29.341 11.48c-2.531.991-3.834 3.967-2.843 6.499l51.628 131.943a4.974 4.974 0 0 0 2.666 2.753 4.965 4.965 0 0 0 3.831.09l29.341-11.48c1.234-.483 2.212-1.43 2.753-2.667s.574-2.598.09-3.832zm-65.283-121.609c-6.138 0-11.145 5.006-11.145 11.144s5.007 11.145 11.145 11.145 11.145-5.007 11.145-11.145-5.006-11.144-11.145-11.144zM88.399 198.733c-6.146-.343-11.405 4.361-11.748 10.507s4.361 11.405 10.507 11.748 11.405-4.361 11.748-10.507-4.361-11.406-10.507-11.748zm1.122-52.576c-3.951-5.36-5.042-11.814-3.072-18.173 5.486-17.715 18.169-31.502 34.285-38.613-6.281-6.409-10.164-15.177-10.164-24.838 0-19.583 15.933-35.516 35.516-35.516S181.6 44.949 181.6 64.532c0 9.657-3.879 18.421-10.154 24.829a61.71 61.71 0 0 1 20.899 15.159 80.628 80.628 0 0 1 34.692-25.634c-9.666-8.22-15.814-20.457-15.814-34.109C211.223 20.087 231.311 0 256.001 0s44.777 20.087 44.777 44.778c0 13.653-6.149 25.89-15.814 34.11a80.63 80.63 0 0 1 34.692 25.635 61.72 61.72 0 0 1 20.899-15.159c-6.275-6.409-10.155-15.173-10.155-24.83 0-19.583 15.933-35.515 35.516-35.515s35.516 15.932 35.516 35.515c0 9.662-3.883 18.43-10.164 24.839 16.115 7.111 28.799 20.897 34.284 38.613 1.97 6.359.879 12.813-3.072 18.173-3.953 5.36-9.795 8.312-16.452 8.312h-69.706v6.7c-.001 15.13-12.311 27.439-27.439 27.439H203.118c-15.13 0-27.438-12.309-27.438-27.439v-6.702h-69.707c-6.66-.001-12.502-2.952-16.452-8.312zm276.387-62.108h.015c10.757-.004 19.509-8.757 19.509-19.515 0-10.761-8.755-19.515-19.516-19.515S346.4 53.773 346.4 64.534c0 10.758 8.751 19.511 19.508 19.515zm-37.305 34.831a80.38 80.38 0 0 1 6.579 19.591h70.846c1.529 0 2.665-.574 3.574-1.806.907-1.23 1.119-2.485.667-3.946-6.05-19.537-23.869-32.664-44.342-32.669h-.019c-14.934.002-28.569 6.975-37.305 18.83zM255.98 73.555h.041c15.859-.011 28.758-12.916 28.758-28.777C284.778 28.91 271.869 16 256.001 16s-28.778 12.91-28.778 28.778c0 15.861 12.899 28.766 28.757 28.777zm-64.3 73.352v14.263c0 6.308 5.131 11.439 11.438 11.439h105.764c6.308 0 11.438-5.132 11.439-11.439v-14.263c-3.709-32.721-31.278-57.342-64.301-57.352H256l-.021-.001c-33.023.011-60.592 24.633-64.299 57.353zm-45.602-62.86h.014c10.758-.004 19.508-8.757 19.508-19.515 0-10.761-8.754-19.516-19.515-19.516s-19.516 8.755-19.516 19.516c0 10.759 8.751 19.511 19.509 19.515zm-43.679 52.617c.908 1.231 2.043 1.805 3.573 1.805h70.847a80.413 80.413 0 0 1 6.578-19.59c-8.736-11.855-22.372-18.828-37.305-18.831h-.019c-20.474.005-38.293 13.132-44.343 32.669-.451 1.46-.239 2.715.669 3.947z" fill="#000000" opacity="1" data-original="#000000"></path></g></svg>
                                                <span>Item Master</span>
                                            </Link>
                                        </li>
                                        {/* <!-- <li>
                                            <a class="drop-link btn-menu" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="18" height="18" x="0" y="0" viewBox="0 0 480 480.584" style="enable-background:new 0 0 512 512" xml:space="preserve" class=""><g><path d="M89.691 130.633c22.774 0 41.235-18.461 41.235-41.235 0-22.77-18.461-41.23-41.235-41.23-22.77 0-41.23 18.46-41.23 41.23.027 22.762 18.469 41.207 41.23 41.235zm0-66.465c13.938 0 25.235 11.297 25.235 25.23 0 13.938-11.297 25.235-25.235 25.235-13.933 0-25.23-11.297-25.23-25.235.016-13.925 11.305-25.21 25.23-25.23zm0 0" fill="#000000" opacity="1" data-original="#000000" class=""></path><path d="m448.43 355.383-3.09-80.719 15.055-20.703-18.254-33.555 3.664-11.277 34.488-16.418v-33.422l-34.488-16.418-3.664-11.277 18.254-33.555-19.649-27.023L403.196 78l-9.598-6.977-4.961-37.863-31.762-10.328-26.277 27.719h-11.86l-26.277-27.719-31.77 10.328-1.55 11.809-111.114-6.617-7.488-7.489-2.07-16.8-29.985-12.461-13.359 10.437H84.293L70.898 1.602l-30 12.445-2.062 16.8-7.68 7.68-16.8 2.067L1.89 70.609 12.348 84v10.855L1.89 108.191l9.914 23.914L.765 273.641a46.139 46.139 0 0 0-.425 5.437v.297H.293V280a47.932 47.932 0 0 0 67.375 43.879l47.344-19.727c-7.672 21.66 1.191 45.715 21.09 57.215l-73.13 23.563c-24.19 4.824-40.785 27.203-38.378 51.754C27 461.234 47.625 479.96 72.293 480h.422l294.578-15.45 4.512 10.817 25.437 5.219 20.649-20.059 5.257-1.742 28.551 3.68 17.242-19.426-7.05-27.91 1.12-5.434 17.458-22.886-8.176-24.641zM275.234 45.246l12.563-4.078 24.062 25.383h25.602l24.062-25.383 12.57 4.078 4.536 34.684 20.719 15.054 34.398-6.398 7.754 10.68L424.797 130l7.918 24.36 31.578 15.038v13.204l-31.578 15.039L424.797 222l16.703 30.727-7.754 10.68-34.398-6.4-20.72 15.056-4.534 34.68-12.57 4.081-24.063-25.383h-25.602l-24.062 25.383-12.563-4.082-4.543-34.664-20.718-15.062-34.403 6.398-7.758-10.68 16.704-30.718-7.91-24.368-31.579-15.039v-13.203l31.578-15.039 7.91-24.367-16.703-30.719 7.758-10.68L249.973 95l20.718-15.078zM20.293 68.121l5.277-12.8 13.035-1.602 15.454-15.43 1.601-13.031 12.8-5.281 10.345 8.078h21.879L110.98 20l12.801 5.281 1.598 13.024 15.418 15.457 13.031 1.597 5.29 12.801-8.083 10.344v21.879l8.082 10.344-5.289 12.8-13.031 1.602-15.457 15.457-1.602 13.031-12.797 5.285-10.343-8.078h-21.84l-10.348 8.078-12.797-5.285-1.601-13.031-15.442-15.54-13.023-1.597-5.277-12.8 8.078-10.344v-21.84zm-4 212.024.094-1.25C18 262.41 32.586 250.293 49.09 251.735s28.77 15.902 27.496 32.421c-1.27 16.516-15.602 28.93-32.129 27.832-16.531-1.101-29.094-15.304-28.164-31.843zm144 7.855c17.672 0 32 14.328 32 32s-14.328 32-32 32c-17.676 0-32-14.328-32-32s14.324-32 32-32zm-120 144a31.993 31.993 0 0 1 18.543-28.937l7.734-2.489A31.97 31.97 0 0 1 97.7 412.82a31.966 31.966 0 0 1 3.16 33.301A31.967 31.967 0 0 1 72.59 464h-.395c-17.636-.055-31.902-14.363-31.902-32zm69.39 30.023a47.858 47.858 0 0 0 9.872-38.523 47.843 47.843 0 0 0-22.672-32.676l78.137-25.136a47.999 47.999 0 0 0 32.835-36.754A48.006 48.006 0 0 0 191 282.62a48.015 48.015 0 0 0-48.781-7.047l-48.606 20.258c7.02-19.977.094-42.191-17.035-54.64-17.133-12.446-40.398-12.169-57.23.68l8-102.063 3.879.48 7.68 7.68 2.066 16.8 30.007 12.415 13.36-10.399h10.855l13.352 10.399 29.922-12.383 2.07-16.801 7.68-7.68 16.8-2.074 12.418-29.988-10.402-13.356V84l10.403-13.36-6.145-14.847 85.8 5.078-1.327 10.16L246.164 78l-37.543-6.984-19.648 27.023 18.246 33.555-3.656 11.254-34.489 16.441v33.422l34.48 16.418 3.665 11.277-18.246 33.555 19.648 27.023L246.164 274l9.602 6.969 4.957 37.87 31.77 10.329 26.28-27.719h11.856l26.277 27.719 31.77-10.328 4.96-37.863 9.598-6.977 26.305 4.895 1.914 50.058-2.043-4.89-25.445-5.231-20.695 20.047-5.258 1.754-28.551-3.68-17.242 19.422 7.035 27.914-1.121 5.43-17.457 22.883 8.199 24.644 27.687 7.875 4.153 3.68v.054zm338.403-59.128-2.594 12.593 5.863 23.254-6.007 6.762-23.785-3.047-12.192 4.047-17.207 16.695-8.855-1.808-9.235-22.125-9.597-8.54-23.067-6.558-2.863-8.594 14.547-19.062 2.582-12.582-5.864-23.258 6.008-6.77 23.785 3.059 12.2-4.05 17.215-16.704 8.855 1.824 9.242 22.137 9.598 8.52 23.066 6.55 2.856 8.594zm0 0" fill="#000000" opacity="1" data-original="#000000" class=""></path><path d="M64.293 280c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zM176.293 320c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zM88.293 432c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zM400.293 360c-22.094 0-40 17.91-40 40s17.906 40 40 40c22.09 0 40-17.91 40-40-.027-22.082-17.922-39.973-40-40zm0 64c-13.254 0-24-10.746-24-24s10.746-24 24-24 24 10.746 24 24-10.746 24-24 24zM320.293 256c44.184 0 80-35.816 80-80s-35.816-80-80-80-80 35.816-80 80c.047 44.164 35.836 79.953 80 80zm0-144c35.344 0 64 28.652 64 64s-28.656 64-64 64c-35.348 0-64-28.652-64-64 .039-35.328 28.668-63.96 64-64zm0 0" fill="#000000" opacity="1" data-original="#000000" class=""></path><path d="M320.293 224c26.508 0 48-21.492 48-48s-21.492-48-48-48c-26.512 0-48 21.492-48 48 .027 26.5 21.5 47.973 48 48zm0-80c17.672 0 32 14.328 32 32s-14.328 32-32 32c-17.676 0-32-14.328-32-32s14.324-32 32-32zM432.293 24c0 13.254 10.746 24 24 24s24-10.746 24-24-10.746-24-24-24-24 10.746-24 24zm32 0a8 8 0 1 1-16 0 8 8 0 0 1 16 0zM224.293 16c0 8.836-7.164 16-16 16s-16-7.164-16-16 7.164-16 16-16 16 7.164 16 16zm0 0" fill="#000000" opacity="1" data-original="#000000" class=""></path></g></svg>
                                                <span>Masters</span>

                                                <svg class="chevron ms-auto dropdown" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                                </svg>
                                            </a>
                                            <ul class="sub-menu inner" style="display: none;">
                                                <li>
                                                    <a href="javascript:void(0)" class="drop-link inner" role="button">
                                                        <span>Vendors</span>
                                                    </a>
                                                </li>
                                                <li>
                                                    <a href="javascript:void(0)" class="drop-link inner" role="button">
                                                        <span>Customers</span>
                                                    </a>
                                                </li>
                                            </ul>
                                        </li> --> */}
                                        <li className="d-none">
                                            <Link to="team.html" className="drop-link" role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 505.4 505.4"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M437.1 233.45c14.8-10.4 24.6-27.7 24.6-47.2 0-31.9-25.8-57.7-57.7-57.7-31.9 0-57.7 25.8-57.7 57.7 0 19.5 9.7 36.8 24.6 47.2-12.7 4.4-24.3 11.2-34.1 20-13.5-11.5-29.4-20.3-46.8-25.5 21.1-12.8 35.3-36.1 35.3-62.6 0-40.4-32.7-73.1-73.1-73.1-40.4 0-73.1 32.8-73.1 73.1 0 26.5 14.1 49.8 35.3 62.6-17.2 5.2-32.9 13.9-46.3 25.2-9.8-8.6-21.2-15.3-33.7-19.6 14.8-10.4 24.6-27.7 24.6-47.2 0-31.9-25.8-57.7-57.7-57.7s-57.7 25.8-57.7 57.7c0 19.5 9.7 36.8 24.6 47.2-39.7 13.7-68.2 51.4-68.2 95.7v6.6c0 .2.2.4.4.4h122.3c-.7 5.5-1.1 11.2-1.1 16.9v6.8c0 29.4 23.8 53.2 53.2 53.2h155c29.4 0 53.2-23.8 53.2-53.2v-6.8c0-5.7-.4-11.4-1.1-16.9H505c.2 0 .4-.2.4-.4v-6.6c-.2-44.4-28.6-82.1-68.3-95.8zm-74.8-47.3c0-23 18.7-41.7 41.7-41.7s41.7 18.7 41.7 41.7c0 22.7-18.3 41.2-40.9 41.7h-1.6c-22.7-.4-40.9-18.9-40.9-41.7zm-167.4-20.8c0-31.5 25.6-57.1 57.1-57.1s57.1 25.6 57.1 57.1c0 30.4-23.9 55.3-53.8 57h-6.6c-29.9-1.7-53.8-26.6-53.8-57zm-135.6 20.8c0-23 18.7-41.7 41.7-41.7s41.7 18.7 41.7 41.7c0 22.7-18.3 41.2-40.9 41.7h-1.6c-22.6-.4-40.9-18.9-40.9-41.7zm66.2 134H16.2c4.5-42.6 40.5-76 84.2-76.3h1.2c20.8.1 39.8 7.8 54.5 20.3-14.4 15.6-25.1 34.8-30.6 56zm241.3 39.8c0 20.5-16.7 37.2-37.2 37.2h-155c-20.5 0-37.2-16.7-37.2-37.2v-6.8c0-62.1 49.6-112.9 111.3-114.7 1.1.1 2.3.1 3.4.1s2.3 0 3.4-.1c61.7 1.8 111.3 52.6 111.3 114.7v6.8zm11.9-39.8c-5.5-21.1-16-40-30.3-55.6 14.8-12.8 34-20.5 55-20.7h1.2c43.7.3 79.7 33.7 84.2 76.3H378.7z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                    </g>
                                                </svg>
                                                <span>Team</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to="report.html"
                                                onClick={() => toggleIsActive('report')}
                                                className={isActive === 'report' ? "drop-link active" : "drop-link"}
                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 48 48"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <g data-name="11 - 20">
                                                            <path
                                                                d="m43.018 42.784-3.9-3.912a9.015 9.015 0 1 0-4.243 4.242l3.911 3.9a2.994 2.994 0 1 0 4.236-4.233zM26.05 39.95a7 7 0 1 1 9.9 0 7.007 7.007 0 0 1-9.9 0zM41.6 45.6a1 1 0 0 1-1.406 0l-3.581-3.573a8.489 8.489 0 0 0 1.414-1.413L41.6 44.2a.991.991 0 0 1 0 1.406z"
                                                                fill="#000000" opacity="1" data-original="#000000"></path>
                                                            <path
                                                                d="M25 44H12a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h17.38a2.973 2.973 0 0 1 .62.067V10a3 3 0 0 0 3 3h5.923a3 3 0 0 1 .077.651V27a1 1 0 0 0 2 0V13.651a4.975 4.975 0 0 0-1.456-3.527l-6.62-6.651A4.965 4.965 0 0 0 29.38 2H12a5.006 5.006 0 0 0-5 5v34a5.006 5.006 0 0 0 5 5h13a1 1 0 0 0 0-2zm12.594-33H33a1 1 0 0 1-1-1V5.38z"
                                                                fill="#000000" opacity="1" data-original="#000000"></path>
                                                            <path
                                                                d="M14 13h11a1 1 0 0 0 0-2H14a1 1 0 0 0 0 2zM34 17H17a1 1 0 0 0 0 2h17a1 1 0 0 0 0-2zM35 23a1 1 0 0 0-1-1H14a1 1 0 0 0 0 2h20a1 1 0 0 0 1-1zM21 28a1 1 0 0 0-1-1h-6a1 1 0 0 0 0 2h6a1 1 0 0 0 1-1zM14 34h4a1 1 0 0 0 0-2h-4a1 1 0 0 0 0 2zM14 39h5a1 1 0 0 0 0-2h-5a1 1 0 0 0 0 2zM31 38a3 3 0 0 1-3-3 1 1 0 0 0-2 0 5.006 5.006 0 0 0 5 5 1 1 0 0 0 0-2z"
                                                                fill="#000000" opacity="1" data-original="#000000"></path>
                                                        </g>
                                                    </g>
                                                </svg>
                                                <span>Reports</span>
                                            </Link>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <Link className="drop-link btn-menu" onClick={toggleHRM} role="button">
                                        <span className="menu-title border-0 p-0 m-0"> HRM </span>
                                        <svg className={HRM ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 20 20" fill="none">
                                            <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </Link>
                                    <ul className="sub-menu" style={{ display: HRM ? 'block' : 'none' }}>
                                        <li>
                                            <Link to={RoutesLink?.attendance_route}
                                                onClick={() => toggleIsActive('attendance')}
                                                className={isActive === 'attendance' ? "drop-link active" : "drop-link"} role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 66 66"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M62.5.7h-59c-1.6 0-3 1.3-3 3v51.6c0 1.6 1.3 3 3 3h6.7v6c0 .6.4 1 1 1s1-.4 1-1V56h13.7v8.3c0 .6.4 1 1 1s1-.4 1-1v-6h34.6c1.6 0 3-1.3 3-3V3.7c0-1.6-1.3-3-3-3zM12.2 54v-1.8c0-.4-.2-.7-.5-.9l-3.9-2.2V44l2.4-1.4V45c0 .6.4 1 1 1s1-.4 1-1V26.5c0-.4.4-1 1-1 .5 0 1 .4 1 1V39.4c0 .6.4 1 1 1 .3 0 .5-.1.7-.3s.3-.4.3-.7v-4.5c0-.5.4-1 1-1 .5 0 1 .4 1 1v4.5c0 .6.4 1 1 1s1-.4 1-1v-4.5c0-.5.4-1 1-1 .5 0 1 .4 1 1v4.5c0 .6.4 1 1 1s1-.4 1-1v-4.5c0-.5.4-1 1-1 .5 0 1 .4 1 1V54zM7.6 7.7h11.2v4H7.6zm0 6h11.2v14.6h-2.6v-1.8c0-1.7-1.4-3-3-3-1.7 0-3 1.5-3 3v1.8H7.6zm55.9 41.6c0 .5-.4 1-1 1H27.9V34.9c0-1.6-1.3-3-3-3-.8 0-1.4.3-2 .8-.5-.5-1.2-.8-2-.8s-1.4.3-2 .8c-.5-.5-1.2-.8-2-.8-.3 0-.7.1-1 .2v-1.8h3.6c.6 0 1-.4 1-1V6.7c0-.6-.4-1-1-1H6.6c-.4 0-.8.3-.9.6-.1.2-.1.3-.1.4v22.5c0 .6.4 1 1 1h3.6v9.9l-3.9 2.4c-.3.2-.5.5-.5.9v6.2c0 .4.2.7.5.9l3.9 2.2v3.6H3.5c-.5 0-1-.4-1-1V3.7c0-.5.4-1 1-1h59c.5 0 1 .4 1 1z"
                                                            fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                        <path
                                                            d="M59.5 5.7H30.7c-.6 0-1 .4-1 1v14.2c0 .6.4 1 1 1h28.8c.6 0 1-.4 1-1V6.7c0-.5-.4-1-1-1zm-1 14.2H31.7V7.7h26.8zM38.3 28.3h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1s-.5-1-1-1zM57.3 28.3h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1s-.4-1-1-1zM47.8 28.3h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1s-.4-1-1-1zM38.3 38.7h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.5-1-1-1zM57.3 38.7h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1zM47.8 38.7h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1zM38.3 49.2h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.5-1-1-1zM57.3 49.2h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1zM47.8 49.2h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1z"
                                                            fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                        <path
                                                            d="M34.1 17.2h4.8c.6 0 1-.4 1-1v-4.8c0-.6-.4-1-1-1h-4.8c-.6 0-1 .4-1 1v4.8c0 .5.4 1 1 1zm1-4.8h2.8v2.8h-2.8zM42.7 17.2h4.8c.6 0 1-.4 1-1v-4.8c0-.6-.4-1-1-1h-4.8c-.6 0-1 .4-1 1v4.8c0 .5.5 1 1 1zm1-4.8h2.8v2.8h-2.8zM51.4 17.2h4.8c.6 0 1-.4 1-1v-4.8c0-.6-.4-1-1-1h-4.8c-.6 0-1 .4-1 1v4.8c0 .5.4 1 1 1zm1-4.8h2.8v2.8h-2.8z"
                                                            fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                    </g>
                                                </svg>
                                                <span>Attendance</span>
                                            </Link>
                                        </li>

                                        <li>
                                            <Link to={RoutesLink?.holidays_route}
                                                onClick={() => toggleIsActive('holiday')}
                                                className={isActive === 'holiday' ? "drop-link active" : "drop-link"}
                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 60 60"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M55 4h-2V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1H5a5.006 5.006 0 0 0-5 5v38a5.006 5.006 0 0 0 5 5h2.01a13.871 13.871 0 0 0-1.91 5.8 2.029 2.029 0 0 0 .52 1.547A2 2 0 0 0 7.1 60H53a2 2 0 0 0 1.477-.651 2.03 2.03 0 0 0 .52-1.548 13.881 13.881 0 0 0-1.91-5.8H55a5.006 5.006 0 0 0 5-5V9a5.006 5.006 0 0 0-5-5Zm-6-1a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM39 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM29 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM19 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM9 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM5 6h2v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h2a3 3 0 0 1 3 3v3H2V9a3 3 0 0 1 3-3Zm2.1 52-.005-.018c.68-7.493 8.781-13.524 19.115-14.743l1.117 2.573a1 1 0 0 0 .918.6.987.987 0 0 0 .4-.083 1 1 0 0 0 .518-1.315l-.849-1.956c.575-.03 1.151-.059 1.737-.059a30.559 30.559 0 0 1 12.33 2.505L41.48 47h-8.9a2 2 0 0 0-1.664.891l-1.7 2.554a1 1 0 1 0 1.664 1.11L32.581 49h8.93l1.7 2.555a1 1 0 1 0 1.664-1.11l-1.649-2.473.949-1.582C49.2 49.119 52.569 53.269 53 58ZM55 50h-3.312a21.742 21.742 0 0 0-6.494-5.3l.709-1.182a1 1 0 0 0-1.715-1.03L43.4 43.8A32.307 32.307 0 0 0 30.046 41c-.874 0-1.737.035-2.589.1L24 33.129a10.824 10.824 0 0 1 3.6-.633.962.962 0 0 0 .878-.432 7.518 7.518 0 0 1 6.36-2.764 1.008 1.008 0 0 0 1.018-.5 1 1 0 0 0-.1-1.128c-3.787-4.6-10.645-5.979-16.679-3.358s-9.7 8.576-8.932 14.482a1 1 0 0 0 .757.843 1.046 1.046 0 0 0 .235.027 1 1 0 0 0 .824-.434 7.522 7.522 0 0 1 6.365-2.764 1.01 1.01 0 0 0 .913-.345 10.927 10.927 0 0 1 2.921-2.2l3.211 7.393C18.159 42.309 12 45.546 8.4 50H5a3 3 0 0 1-3-3V14h56v33a3 3 0 0 1-3 3ZM18.878 33.727c-1.039-3.15-.675-5.758 1.033-7.493 2.414-.061 4.584 1.451 6.18 4.36a14.313 14.313 0 0 0-7.213 3.133Zm9.029-3.972a11.084 11.084 0 0 0-4.32-4.633 12.065 12.065 0 0 1 9.071 2.3 9.608 9.608 0 0 0-4.751 2.331ZM17.026 34.48a9.62 9.62 0 0 0-4.945 1.881 12.059 12.059 0 0 1 4.5-8.194 11.081 11.081 0 0 0 .445 6.313Z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M50.046 24H48.9a3.915 3.915 0 0 0-.425-1.019l.809-.809a1 1 0 1 0-1.414-1.414l-.809.809a3.915 3.915 0 0 0-1.019-.425V20a1 1 0 1 0-2 0v1.142a3.915 3.915 0 0 0-1.019.425l-.809-.809a1 1 0 0 0-1.414 1.414l.809.809A3.915 3.915 0 0 0 41.188 24h-1.142a1 1 0 1 0 0 2h1.142a3.915 3.915 0 0 0 .425 1.019l-.809.809a1 1 0 1 0 1.414 1.414l.809-.809a3.915 3.915 0 0 0 1.019.425V30a1 1 0 0 0 2 0v-1.142a3.915 3.915 0 0 0 1.019-.425l.809.809a1 1 0 1 0 1.414-1.414l-.809-.809A3.915 3.915 0 0 0 48.9 26h1.142a1 1 0 0 0 0-2Zm-5 3a2 2 0 1 1 2-2 2 2 0 0 1-2 2Z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Holidays</span>
                                            </Link>
                                        </li>
                                        {/* <li>
                                            <Link to={RoutesLink?.reimbursement_route}
                                                onClick={() => toggleIsActive('reimbursement')}
                                                className={isActive === 'reimbursement' ? "drop-link active" : "drop-link"}
                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 64 64"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M56.716 44.31c0-.513-.098-1.016-.289-1.485a3.974 3.974 0 0 0-4.675-2.393c-.045.012-.09.026-.133.044L38.376 45.93c-.015-.106-.02-.213-.047-.318a3.148 3.148 0 0 0-3.445-2.366c-.045.006-.09.015-.134.026l-13.84 3.706-4.929-.824V43a1 1 0 0 0-1-1H10a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1h4.981a1 1 0 0 0 1-1v-2h6.788l6.175 2.325c.556.21 1.14.315 1.729.315.786 0 1.579-.188 2.325-.568l21.51-13.177a3.974 3.974 0 0 0 2.208-3.585zM13.981 60H11V44h2.981zM53.54 46.147 32.022 59.328c-.734.372-1.6.417-2.373.126l-6.346-2.39a.988.988 0 0 0-.352-.064h-6.97v-8.819l4.812.805c.14.023.286.017.424-.021l13.975-3.742a1.153 1.153 0 0 1 1.058 1.763c-.165.267-.423.45-.727.519a.986.986 0 0 0-.128.038l-6.813 2.52.693 1.875 6.756-2.498a3.099 3.099 0 0 0 1.666-1.067l14.612-6.018c.94-.205 1.901.3 2.26 1.211.097.24.146.49.146.744.001.77-.42 1.458-1.175 1.837zM32 26c-.551 0-1-.448-1-1h-2c0 1.302.839 2.402 2 2.816V29h2v-1.184A2.996 2.996 0 0 0 35 25c0-1.654-1.346-3-3-3a1.001 1.001 0 0 1 0-2c.551 0 1 .448 1 1h2a2.996 2.996 0 0 0-2-2.816V17h-2v1.184A2.996 2.996 0 0 0 29 21c0 1.654 1.346 3 3 3a1.001 1.001 0 0 1 0 2z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M19 13a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1h26a1 1 0 0 0 1-1V14a1 1 0 0 0-1-1zm25 18H20V15h24z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M8.006 34.891c-.042.389.145.766.479.967l5 3a1.007 1.007 0 0 0 .92.056 1 1 0 0 0 .576-.718l1-5a.998.998 0 0 0-1.297-1.144l-2.193.731A20.825 20.825 0 0 1 11 25c0-11.579 9.42-21 21-21s21 9.421 21 21c0 4.501-1.403 8.792-4.056 12.408l1.612 1.184A22.824 22.824 0 0 0 55 25C55 12.317 44.682 2 32 2S9 12.317 9 25c0 2.916.535 5.741 1.589 8.417l-1.906.635a.998.998 0 0 0-.677.839zm5.676-.398-.384 1.919-1.919-1.151z"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Reimbursement</span>
                                            </Link>
                                        </li> */}
                                        <li>
                                            <Link to="pay-slip.html"
                                                onClick={() => toggleIsActive('pay-slip')}
                                                className={isActive === 'pay-slip' ? "drop-link active" : "drop-link"}

                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="18" x="0" y="0" viewBox="0 0 48 48"
                                                    style={{ enableBackground: "new 0 0 512 512" }}>
                                                    <g>
                                                        <path
                                                            d="M41.179 11.116h-38.4A2.782 2.782 0 0 0 0 13.895v24.252a2.782 2.782 0 0 0 2.779 2.78h38.4a2.782 2.782 0 0 0 2.779-2.78V13.895a2.782 2.782 0 0 0-2.78-2.78zm1.263 27.031c0 .697-.566 1.264-1.263 1.264h-38.4a1.264 1.264 0 0 1-1.263-1.264V13.895c0-.697.566-1.263 1.263-1.263h38.4c.697 0 1.263.566 1.263 1.263v24.252zM48 9.853v24.252c0 .998-.518 1.896-1.387 2.403a.757.757 0 1 1-.764-1.309c.398-.232.635-.64.635-1.094V9.853c0-.697-.566-1.264-1.263-1.264h-38.4c-.453 0-.861.238-1.093.635a.759.759 0 0 1-1.31-.764 2.758 2.758 0 0 1 2.403-1.386h38.4A2.782 2.782 0 0 1 48 9.853zM4.8 23.747h9.095c.418 0 .758-.34.758-.757v-7.074a.758.758 0 0 0-.758-.758H4.8a.758.758 0 0 0-.758.758v7.074c0 .418.34.757.758.757zm5.305-1.515V20.21h3.032v2.02zm3.032-3.537h-3.032v-2.021h3.032zm-7.58-2.021H8.59v5.558H5.558zm33.6-1.516H25.012a.758.758 0 0 0-.758.758v4.042c0 .418.34.758.758.758h14.147c.418 0 .758-.34.758-.758v-4.042a.758.758 0 0 0-.758-.758zM38.4 19.2H25.768v-2.526H38.4zM4.042 29.053c0-.419.34-.758.758-.758h4.042a.758.758 0 0 1 0 1.516H4.8a.758.758 0 0 1-.758-.758zm13.642 0c0 .418-.34.758-.758.758h-4.042a.758.758 0 0 1 0-1.516h4.042c.419 0 .758.34.758.758zm8.084 0c0 .418-.34.758-.758.758h-4.042a.758.758 0 0 1 0-1.516h4.042c.419 0 .758.34.758.758zm8.085 0c0 .418-.34.758-.758.758h-4.042a.758.758 0 0 1 0-1.516h4.042c.418 0 .758.34.758.758zm-16.169 5.052c0 .419-.34.758-.758.758H4.8a.758.758 0 0 1 0-1.516h12.126c.419 0 .758.34.758.758z"
                                                            fill="#000000" opacity="1" data-original="#000000"></path>
                                                    </g>
                                                </svg>
                                                <span>Pay slip</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to="Reports.html"
                                                onClick={() => toggleIsActive('Reports')}
                                                className={isActive === 'Reports' ? "drop-link active" : "drop-link"}

                                                role="button">
                                                <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                    width="18" height="20" x="0" y="0" viewBox="0 0 460 460"
                                                    style={{ enableBackground: "new 0 0 512 512" }} className="">
                                                    <g>
                                                        <path
                                                            d="M7.516 460h333.941c4.418 0 7.543-4.078 7.543-8.5V278.871l106.922-106.93a7.999 7.999 0 0 0 .105-11.304l-42.547-42.602a8.246 8.246 0 0 0-11.507-.008L349 170.68V89.45a7.795 7.795 0 0 0-2.117-5.66l-80.836-81.2A7.936 7.936 0 0 0 260.5 0H7.516C3.102 0 0 4.078 0 8.496v443.008C0 455.922 3.102 460 7.516 460zm255.398-173.387 24.649 24.649-37.028 12.379zm39.441 16.813-31.609-31.606 101.516-101.289 31.383 31.383zM407.875 135l31.3 31.3-24.234 24.29-31.351-31.36zM269 27.809 322.145 81H269zM16 16h237v73.45a7.204 7.204 0 0 0 2.09 5.425A7.203 7.203 0 0 0 260.5 97H333v89.645l-79.453 79.5a7.154 7.154 0 0 0-1.82 2.949L240.445 303H67.7a8 8 0 0 0 0 16h167.403l-4.82 14.602a7.843 7.843 0 0 0 1.929 8.097c.562.563 1.2 1.301 1.875 1.301H67.699a8 8 0 0 0 0 16h213.004c4.422 0 8-3.582 8-8s-3.578-8-8-8h-38.691l62.886-20.848a7.009 7.009 0 0 0 2.903-1.843L333 294.906V444H16zm0 0"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                        <path
                                                            d="M280.703 383H67.7a8 8 0 0 0 0 16h213.004c4.422 0 8-3.582 8-8s-3.578-8-8-8zM67.7 279h113.003c4.422 0 8-3.582 8-8s-3.578-8-8-8H67.7a8 8 0 0 0 0 16zm0 0"
                                                            fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                    </g>
                                                </svg>
                                                <span>Reports</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link className="drop-link btn-menu" onClick={toggleMAsters} role="button">
                                                <span className="menu-title border-0 p-0 m-0"> Masters </span>
                                                <svg className={Masters ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                                    viewBox="0 0 20 20" fill="none">
                                                    <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                        stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            </Link>
                                            <ul className="sub-menu" style={{ display: Masters ? 'block' : 'none' }}>
                                                {/* <li>
                                                    <Link to={RoutesLink?.department_route}
                                                        onClick={() => toggleIsActive('department')}
                                                        className={isActive === 'department' ? "drop-link active" : "drop-link"}

                                                        role="button">
                                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                            width="18" height="18" x="0" y="0"
                                                            viewBox="0 0 64 64" style={{ enableBackground: "new 0 0 512 512" }}
                                                            className="">
                                                            <g>
                                                                <path
                                                                    d="M27 45a5 5 0 1 0 5-5 5 5 0 0 0-5 5zm8 0a3 3 0 1 1-3-3 3 3 0 0 1 3 3zM16 45a5 5 0 1 0 5-5 5 5 0 0 0-5 5zm8 0a3 3 0 1 1-3-3 3 3 0 0 1 3 3zM38 45a5 5 0 1 0 5-5 5 5 0 0 0-5 5zm8 0a3 3 0 1 1-3-3 3 3 0 0 1 3 3zM49 45a5 5 0 1 0 5-5 5 5 0 0 0-5 5zm5-3a3 3 0 1 1-3 3 3 3 0 0 1 3-3zM5 45a5 5 0 1 0 5-5 5 5 0 0 0-5 5zm8 0a3 3 0 1 1-3-3 3 3 0 0 1 3 3zM61 62v-6a5 5 0 0 0-5-5h-4a5 5 0 0 0-3.5 1.44A5 5 0 0 0 45 51h-4a5 5 0 0 0-3.5 1.44A5 5 0 0 0 34 51h-4a5 5 0 0 0-3.5 1.44A5 5 0 0 0 23 51h-4a5 5 0 0 0-3.5 1.44A5 5 0 0 0 12 51H8a5 5 0 0 0-5 5v6a1 1 0 0 0 1 1h56a1 1 0 0 0 1-1zm-47-6v5H5v-5a3 3 0 0 1 3-3h4a3 3 0 0 1 2.35 1.19A4.94 4.94 0 0 0 14 56zm11 0v5h-9v-5a3 3 0 0 1 3-3h4a3 3 0 0 1 2.35 1.19A4.94 4.94 0 0 0 25 56zm12 5H27v-5a3 3 0 0 1 3-3h1.67L31 55l1 4 1-4-.67-2H34a3 3 0 0 1 3 3zm11 0h-9v-5a4.94 4.94 0 0 0-.35-1.81A3 3 0 0 1 41 53h4a3 3 0 0 1 3 3zm11 0h-9v-5a4.87 4.87 0 0 0-.35-1.82A3 3 0 0 1 52 53h4a3 3 0 0 1 3 3zM19 21h2v2h-2zM19 25h2v2h-2zM19 29h2v2h-2zM27 16h2v2h-2zM27 20h2v2h-2zM27 24h2v2h-2zM31 16h2v2h-2zM29 9h2v2h-2zM33 9h2v2h-2zM31 20h2v2h-2zM31 24h2v2h-2zM35 16h2v2h-2zM35 20h2v2h-2zM35 24h2v2h-2zM43 21h2v2h-2zM43 25h2v2h-2zM43 29h2v2h-2z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                                <path
                                                                    d="M38 6H26a1 1 0 0 0-1 1v5h-1a1 1 0 0 0-1 1v3.21l-7.23 1.72a1 1 0 0 0-.77 1V36h-3v2h40v-2h-3V18.9a1 1 0 0 0-.77-1L41 16.21V13a1 1 0 0 0-1-1h-1V7a1 1 0 0 0-1-1zM23 36h-2v-3h-2v3h-2V19.69l6-1.42zm18-17.73 6 1.42V36h-2v-3h-2v3h-2zM27 8h10v4H27zm7 28h-4v-6h4zm5 0h-3v-7a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v7h-3V14h14zM3.06 30.07l-2-.13C1 30.62 1 31.31 1 32h2c0-.65 0-1.29.06-1.93zM12.89 10.19l-1.32-1.51a30.88 30.88 0 0 0-2.86 2.86l1.5 1.33a28.25 28.25 0 0 1 2.68-2.68zM7.9 15.87l-1.66-1.12a31.44 31.44 0 0 0-2 3.51l1.8.89a28.84 28.84 0 0 1 1.86-3.28zM60.43 26.26l2-.39a30.87 30.87 0 0 0-1-3.91l-1.89.65a28.9 28.9 0 0 1 .89 3.65zM4.54 22.65 2.65 22a29.42 29.42 0 0 0-1 3.92l2 .39a28.6 28.6 0 0 1 .89-3.66zM19.17 6l-.88-1.79a31.37 31.37 0 0 0-3.51 2l1.11 1.66A29.73 29.73 0 0 1 19.17 6zM53.77 12.84l1.5-1.32a31 31 0 0 0-2.87-2.86l-1.31 1.51a28.14 28.14 0 0 1 2.68 2.67zM41.93 2.63a29.28 29.28 0 0 0-3.91-1l-.39 2a28.57 28.57 0 0 1 3.66 1zM49.19 6.2a31.37 31.37 0 0 0-3.51-2L44.79 6a27.47 27.47 0 0 1 3.29 1.89zM59.78 18.22a30.68 30.68 0 0 0-2-3.5l-1.66 1.12A29 29 0 0 1 58 19.11zM26.33 3.55l-.39-2A31.48 31.48 0 0 0 22 2.64l.65 1.89a30.35 30.35 0 0 1 3.68-.98zM33.87 3.06l.13-2a32.19 32.19 0 0 0-4.05 0l.13 2a30 30 0 0 1 3.79 0zM60.94 30.07c0 .64.06 1.28.06 1.93h2c0-.69 0-1.38-.07-2.06z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                            </g>
                                                        </svg>
                                                        <span>Department</span>
                                                    </Link>
                                                </li> */}
                                                {/* <li>
                                                    <Link to={RoutesLink?.designation_route}
                                                        onClick={() => toggleIsActive('designation')}
                                                        className={isActive === 'designation' ? "drop-link active" : "drop-link"}

                                                        role="button">
                                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                            width="18" height="18" x="0" y="0"
                                                            viewBox="0 0 66 66" style={{ enableBackground: "new 0 0 512 512" }}
                                                            className="">
                                                            <g>
                                                                <path
                                                                    d="M58.08 10.05c1.31 0 2.37-1.06 2.37-2.37s-1.06-2.37-2.37-2.37-2.37 1.06-2.37 2.37 1.07 2.37 2.37 2.37zm0-3.05c.38 0 .69.31.69.69s-.31.69-.69.69c-.39 0-.69-.3-.69-.69s.31-.69.69-.69z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                                <path
                                                                    d="M60.71 1.48H11.99c-1.85 0-3.36 1.51-3.36 3.36v5.13H5.29c-1.85 0-3.36 1.51-3.36 3.36v42.13c0 1.85 1.51 3.36 3.36 3.36h22.33v4.86c0 .46.38.84.84.84h15.78c.46 0 .84-.38.84-.84v-4.86h8.93c1.85 0 3.36-1.51 3.36-3.36v-5.13h3.34c1.85 0 3.36-1.51 3.36-3.36V4.84c0-1.85-1.51-3.36-3.36-3.36zm-50.4 3.36c0-.93.76-1.68 1.68-1.68h48.72c.93 0 1.68.76 1.68 1.68v7.37H30.51l-3.75-3.89c-.29-.3-.69-.47-1.1-.47H17.4c-.84 0-1.53.69-1.53 1.53v2.83h-5.56zM5.29 57.15c-.93 0-1.68-.76-1.68-1.68V13.33c0-.93.76-1.68 1.68-1.68h3.34v35.33c0 1.85 1.51 3.36 3.36 3.36h18.24c.33.64.48 1.36.42 2.1h-1.3c-.96 0-1.73.78-1.73 1.73v2.98zm30.23-17.59c.17-.3.48-.49.83-.49.52 0 .94.42.94.94 0 .34-.18.65-.47.82-.32.19-.7.17-1.09-.05a.278.278 0 0 1-.12-.12c-.32-.51-.19-.91-.09-1.1zm-.01-2.01a2.6 2.6 0 0 0-1.45 1.2c-.48.86-.42 1.91.14 2.81.18.28.42.51.71.68.46.26.96.4 1.45.4.46 0 .91-.12 1.31-.35.8-.47 1.3-1.34 1.3-2.27 0-1.15-.75-2.12-1.78-2.47v-4.32l7.43 13.41c-.01.01-.02.03-.05.04-2.63.68-4.35 3.14-4.21 5.76h-8.01c.15-2.63-1.58-5.09-4.26-5.84l7.42-13.37zm7.89 25.29H29.3l.05-8.73h14c.03 0 .05.02.05.05zm12.29-7.38c0 .93-.76 1.68-1.68 1.68h-8.93v-2.98c0-.96-.78-1.73-1.73-1.73h-1.3c-.06-.74.1-1.46.42-2.1h13.22zm5.02-6.8H44.1c.27-.15.56-.28.88-.36.53-.14.96-.51 1.18-1.01.21-.49.18-1.04-.08-1.51l-8.6-15.49c-.22-.41-.65-.67-1.12-.67-.48.01-.91.25-1.14.66l-8.61 15.5c-.26.47-.28 1.03-.07 1.52.21.5.64.86 1.17 1 .32.08.6.21.88.36h-16.6c-.93 0-1.68-.76-1.68-1.68V13.89h5.7c.84 0 1.53-.69 1.53-1.53V9.53h8.05l3.75 3.89c.29.3.69.47 1.1.47h31.95v33.08c0 .93-.75 1.69-1.68 1.69z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                                <path
                                                                    d="M52.36 10.05c1.31 0 2.37-1.06 2.37-2.37s-1.06-2.37-2.37-2.37-2.37 1.06-2.37 2.37 1.06 2.37 2.37 2.37zm0-3.05c.38 0 .69.31.69.69s-.31.69-.69.69c-.39 0-.69-.3-.69-.69s.31-.69.69-.69zM46.63 10.05c1.31 0 2.37-1.06 2.37-2.37s-1.06-2.37-2.37-2.37-2.37 1.06-2.37 2.37 1.06 2.37 2.37 2.37zm0-3.05c.38 0 .69.31.69.69s-.31.69-.69.69c-.39 0-.69-.3-.69-.69s.31-.69.69-.69zM52.54 21.21c-.49.49-.8 1.11-.92 1.77H39.87v-1.32c0-.9-.73-1.63-1.63-1.63h-3.79c-.9 0-1.63.73-1.63 1.63v1.32H21.08c-.11-.65-.42-1.27-.92-1.78-1.27-1.25-3.44-1.23-4.68.01-.63.63-.97 1.46-.97 2.34 0 .89.35 1.73.97 2.34.63.65 1.47.98 2.34.98.55 0 1.11-.13 1.64-.4a2.95 2.95 0 0 0 1.32-1.36c.07-.15.13-.3.18-.45h8.15c-4.43 2.16-7.77 6.22-8.93 11.12h-1.3c-.9 0-1.63.73-1.63 1.63v3.79c0 .9.73 1.63 1.63 1.63h3.79c.9 0 1.63-.73 1.63-1.63v-3.79c0-.9-.73-1.63-1.63-1.63h-.76c1.4-5.28 5.63-9.42 10.94-10.7v.36c0 .9.73 1.63 1.63 1.63h3.79c.9 0 1.63-.73 1.63-1.63v-.36c5.3 1.28 9.53 5.42 10.94 10.7h-.69c-.9 0-1.63.73-1.63 1.63v3.79c0 .9.73 1.63 1.63 1.63h3.79c.9 0 1.63-.73 1.63-1.63v-3.79c0-.9-.73-1.63-1.63-1.63h-1.37c-1.17-4.9-4.51-8.97-8.93-11.12h8.17c.16.46.42.88.77 1.23.63.65 1.47.98 2.34.98.55 0 1.11-.13 1.64-.4a2.95 2.95 0 0 0 1.32-1.36c.65-1.37.42-2.86-.61-3.91-1.31-1.24-3.48-1.23-4.73.01zM19.25 24.4c-.12.24-.32.45-.56.57-.87.44-1.62.16-2.03-.27-.31-.3-.48-.71-.48-1.15 0-.43.17-.84.48-1.16.31-.31.72-.48 1.16-.48.43 0 .85.17 1.15.47.52.54.62 1.29.28 2.02zm3.4 16.75-3.74.05-.05-3.74h3.74zm31.26 0-3.74.05-.05-3.74h3.74zm-19.4-15.7-.02-1.26c.02-.09.02-.18.01-.28v-.01c0-.03.02-.05.02-.08 0-.05-.02-.09-.03-.13l-.03-1.97h3.74l.03 1.97c-.01.05-.03.09-.03.14 0 .03.01.05.02.08v.01c-.02.13-.01.25.02.36l.02 1.13zm21.81-1.05c-.12.24-.32.45-.56.57-.87.44-1.62.16-2.03-.27-.31-.3-.48-.71-.48-1.15 0-.43.17-.84.48-1.16.31-.31.72-.48 1.16-.48.43 0 .85.17 1.15.47.52.54.62 1.29.28 2.02z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                            </g>
                                                        </svg>
                                                        <span>Designation</span>
                                                    </Link>
                                                </li> */}
                                                <li>
                                                    <Link to={RoutesLink?.shiftAndTimeCongiguration_route}
                                                        onClick={() => toggleIsActive('shift-time-configuration')}
                                                        className={isActive === 'shift-time-configuration' ? "drop-link active" : "drop-link"}

                                                        role="button">
                                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                            width="18" height="18" x="0" y="0"
                                                            viewBox="0 0 512 512" style={{ enableBackground: "new 0 0 512 512" }}
                                                            className="">
                                                            <g>
                                                                <path
                                                                    d="M371 239c-72.785 0-132 59.215-132 132s59.215 132 132 132 132-59.215 132-132-59.215-132-132-132zm0 250c-65.065 0-118-52.935-118-118 0-65.066 52.935-118 118-118s118 52.935 118 118-52.935 118-118 118zm.002-135.165c20.147 0 36.539-16.391 36.539-36.538 0-20.145-16.392-36.534-36.539-36.534s-36.538 16.389-36.538 36.534c0 20.147 16.39 36.538 36.538 36.538zm0-59.072c12.428 0 22.539 10.108 22.539 22.534 0 12.428-10.111 22.538-22.539 22.538s-22.538-10.11-22.538-22.538c0-12.425 10.11-22.534 22.538-22.534zm0 62.094c-39.101 0-70.912 31.809-70.912 70.908 0 7.93 4.26 18.951 24.554 26.422 12.346 4.546 28.809 7.049 46.356 7.049s34.011-2.503 46.356-7.049c20.294-7.471 24.554-18.493 24.554-26.422 0-39.098-31.809-70.908-70.908-70.908zm-.002 90.38c-31.38 0-56.91-8.735-56.91-19.472 0-31.379 25.53-56.908 56.912-56.908 31.379 0 56.908 25.529 56.908 56.908 0 10.737-25.53 19.472-56.91 19.472zm-151.284-39.321a7 7 0 0 1-7 7H144.07c-25.643 0-46.505-20.861-46.505-46.504v-49.467l-16.101 19.601a7.001 7.001 0 0 1-10.819-8.887l28.51-34.708a7 7 0 0 1 10.817-.001l28.516 34.708a7 7 0 0 1-10.816 8.888l-16.107-19.605v49.471c0 17.923 14.582 32.504 32.505 32.504h68.646a7 7 0 0 1 7 7zM292.38 104.5a7 7 0 0 1 7-7h68.55c25.643 0 46.504 20.862 46.504 46.504v49.051l16.101-19.601a7.001 7.001 0 0 1 10.819 8.887l-28.51 34.708a7 7 0 0 1-10.818.001l-28.515-34.708a7.001 7.001 0 0 1 10.819-8.888l16.105 19.604v-49.053c0-17.923-14.581-32.504-32.504-32.504h-68.55a7.002 7.002 0 0 1-7.001-7.001zM140.999 9C68.215 9 9 68.215 9 141s59.215 132 131.999 132c72.785 0 132-59.215 132-132s-59.215-132-132-132zm0 250C75.935 259 23 206.065 23 141S75.935 23 140.999 23c65.065 0 118 52.935 118 118s-52.935 118-118 118zm0-223.536C82.807 35.464 35.464 82.807 35.464 141c0 58.192 47.343 105.536 105.535 105.536S246.535 199.192 246.535 141c0-58.193-47.344-105.536-105.536-105.536zm0 197.072c-50.473 0-91.535-41.063-91.535-91.536s41.063-91.536 91.535-91.536c50.474 0 91.536 41.063 91.536 91.536s-41.062 91.536-91.536 91.536zm32.674-72.673a6.998 6.998 0 0 1-9.562 2.562l-17.828-10.293A12.228 12.228 0 0 1 141 153.34c-6.797 0-12.34-5.544-12.34-12.34 0-4.2 2.12-7.916 5.34-10.147v-32.6a7 7 0 1 1 14 0v32.6a12.346 12.346 0 0 1 5.289 9.159l17.821 10.289a7.001 7.001 0 0 1 2.563 9.562zM134 70.118v-4.909a7 7 0 1 1 14 0v4.909a7 7 0 1 1-14 0zm65.541 12.339a7 7 0 0 1 .001 9.9l-3.471 3.472a6.976 6.976 0 0 1-4.95 2.051 7 7 0 0 1-4.95-11.949l3.471-3.472a6.999 6.999 0 0 1 9.899-.002zM223.791 141a7 7 0 0 1-7 7h-4.909a7 7 0 1 1 0-14h4.909a7 7 0 0 1 7 7zm-24.249 48.643a7 7 0 0 1-9.9 9.898l-3.471-3.471a7 7 0 0 1 9.9-9.898zM148 211.881v4.91a7 7 0 1 1-14 0v-4.91a7 7 0 1 1 14 0zm-52.171-25.709a7 7 0 0 1-.001 9.899l-3.472 3.471a6.975 6.975 0 0 1-4.949 2.05 7 7 0 0 1-4.949-11.95l3.472-3.471a6.999 6.999 0 0 1 9.899.001zM77.118 141a7 7 0 0 1-7 7h-4.909a7 7 0 1 1 0-14h4.909a7 7 0 0 1 7 7zm18.711-55.071a7 7 0 0 1-9.9 9.899l-3.472-3.472a7 7 0 0 1 9.9-9.899z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                            </g>
                                                        </svg>
                                                        <span>Shift & Time Config</span>
                                                    </Link>
                                                </li>
                                                <li>
                                                    <Link to={RoutesLink?.employeeOnBoard_route}
                                                        onClick={() => toggleIsActive('employee-onboard')}
                                                        className={isActive === 'employee-onboard' ? "drop-link active" : "drop-link"}

                                                        role="button">
                                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                            width="18" height="18" x="0" y="0"
                                                            viewBox="0 0 512 512" style={{ enableBackground: "new 0 0 512 512" }}
                                                            className="">
                                                            <g>
                                                                <path
                                                                    d="M196.203 169.713h119.594a5.994 5.994 0 0 0 5.995-5.995v-28.035c0-.073 0-.146-.004-.219-.696-19.087-14.876-34.024-37.859-40.808 6.017-6.669 9.687-15.494 9.687-25.161 0-20.749-16.868-37.63-37.602-37.63s-37.63 16.881-37.63 37.63c0 9.669 3.673 18.495 9.696 25.164-22.99 6.785-37.173 21.72-37.868 40.807a5.252 5.252 0 0 0-.004.218v28.035a5.994 5.994 0 0 0 5.995 5.995zm59.811-125.858c14.122 0 25.611 11.502 25.611 25.64s-11.489 25.611-25.611 25.611-25.64-11.489-25.64-25.611 11.502-25.64 25.64-25.64zm-53.815 91.942c.755-18.826 20.235-28.698 38.843-31.814 4.592 1.998 9.653 3.113 14.973 3.113s10.375-1.115 14.964-3.114c18.596 3.116 38.068 12.988 38.824 31.815v21.926H202.199zM97.722 94.655c6.017-6.668 9.686-15.493 9.686-25.161 0-20.749-16.868-37.63-37.602-37.63s-37.602 16.881-37.602 37.63c0 9.667 3.669 18.492 9.686 25.161-23 6.785-37.191 21.723-37.886 40.81a5.252 5.252 0 0 0-.004.218v28.035a5.994 5.994 0 0 0 5.995 5.995h119.622a5.994 5.994 0 0 0 5.995-5.995v-28.035c0-.073 0-.146-.004-.219-.696-19.086-14.887-34.024-37.887-40.809zm-27.916-50.8c14.122 0 25.611 11.502 25.611 25.64S83.928 95.106 69.806 95.106 44.195 83.617 44.195 69.495s11.489-25.64 25.611-25.64zm53.816 113.868H15.99v-21.926c.755-18.824 20.241-28.698 38.852-31.815 4.589 1.999 9.647 3.114 14.964 3.114s10.375-1.115 14.964-3.114c18.61 3.118 38.096 12.99 38.853 31.815v21.926zm324.566 42.817v145.275c0 18.357-14.922 33.293-33.265 33.293h-25.171a5.995 5.995 0 1 1 0-11.99h25.171c11.73 0 21.274-9.557 21.274-21.303V200.54a5.995 5.995 0 1 1 11.99 0zM128.242 373.113a5.994 5.994 0 0 1-5.995 5.995H97.103c-18.357 0-33.293-14.936-33.293-33.293V200.54a5.995 5.995 0 1 1 11.99 0v145.275c0 11.746 9.557 21.303 21.303 21.303h25.144a5.994 5.994 0 0 1 5.995 5.995zm127.771-178.568a5.994 5.994 0 0 1 5.995 5.995v40.932a5.995 5.995 0 1 1-11.99 0V200.54a5.994 5.994 0 0 1 5.995-5.995zm251.982-59.081c-.696-19.086-14.879-34.021-37.869-40.806 6.023-6.669 9.696-15.495 9.696-25.164 0-20.749-16.881-37.63-37.63-37.63s-37.602 16.881-37.602 37.63c0 9.668 3.67 18.493 9.687 25.162-22.983 6.785-37.163 21.721-37.858 40.809a5.252 5.252 0 0 0-.004.218v28.035a5.994 5.994 0 0 0 5.995 5.995h119.594a5.994 5.994 0 0 0 5.995-5.995v-28.035c0-.073 0-.146-.004-.219zm-65.803-91.609c14.138 0 25.64 11.502 25.64 25.64s-11.502 25.611-25.64 25.611-25.611-11.489-25.611-25.611 11.489-25.64 25.611-25.64zm53.816 113.868H388.404v-21.926c.755-18.827 20.227-28.699 38.823-31.815 4.589 1.999 9.647 3.114 14.964 3.114s10.381-1.115 14.973-3.113c18.607 3.117 38.087 12.987 38.844 31.814zM349.867 346.652h-5.424a92.294 92.294 0 0 0-7.199-17.392l3.802-3.817a13.013 13.013 0 0 0 3.882-9.274 13.084 13.084 0 0 0-3.86-9.345l-18.795-18.795c-5.127-5.125-13.471-5.124-18.598 0l-3.84 3.841a91.413 91.413 0 0 0-17.39-7.191v-5.449c0-7.245-5.894-13.139-13.139-13.139h-26.589c-7.261 0-13.167 5.894-13.167 13.139v5.438a90.663 90.663 0 0 0-17.392 7.2l-3.839-3.839c-5.102-5.103-13.433-5.111-18.604.007l-18.809 18.809c-5.091 5.143-5.08 13.476.021 18.576l3.839 3.839a90.583 90.583 0 0 0-7.2 17.392h-5.438c-7.245 0-13.139 5.906-13.139 13.167v26.589c0 7.245 5.894 13.139 13.139 13.139h5.437a90.365 90.365 0 0 0 7.202 17.392l-3.838 3.838c-2.477 2.476-3.841 5.778-3.841 9.3s1.364 6.824 3.84 9.299l18.793 18.793a13.077 13.077 0 0 0 9.31 3.862h.034c3.514-.009 6.808-1.389 9.246-3.853l3.847-3.832a92.252 92.252 0 0 0 17.393 7.2v5.423c0 7.261 5.906 13.167 13.167 13.167h26.589c7.245 0 13.139-5.906 13.139-13.167v-5.423a92.231 92.231 0 0 0 17.392-7.2l3.816 3.802c2.479 2.504 5.789 3.883 9.321 3.883s6.842-1.379 9.293-3.854l9.41-9.383.014-.013 9.354-9.384c2.503-2.478 3.882-5.788 3.882-9.32s-1.379-6.843-3.852-9.29l-3.831-3.847a92.126 92.126 0 0 0 7.201-17.393h5.421c7.261 0 13.167-5.894 13.167-13.139v-26.589c0-7.261-5.906-13.167-13.167-13.167zm1.177 39.756c0 .622-.539 1.148-1.177 1.148H339.86a5.994 5.994 0 0 0-5.825 4.577 80.13 80.13 0 0 1-9.39 22.684 5.995 5.995 0 0 0 .876 7.343l7.089 7.117c.271.268.327.582.327.799s-.057.531-.355.827l-18.809 18.809c-.268.271-.583.327-.799.327s-.532-.057-.829-.356l-7.086-7.059a5.994 5.994 0 0 0-7.344-.877 80.353 80.353 0 0 1-22.688 9.392 5.994 5.994 0 0 0-4.573 5.824v10.006c0 .638-.526 1.177-1.148 1.177h-26.589c-.627 0-1.177-.55-1.177-1.177v-10.006a5.994 5.994 0 0 0-4.573-5.824 80.33 80.33 0 0 1-22.688-9.392 5.99 5.99 0 0 0-7.342.876l-7.118 7.09a1.082 1.082 0 0 1-.784.326c-.251-.021-.544-.06-.834-.349l-18.795-18.795c-.284-.284-.327-.636-.327-.819s.043-.535.328-.82l7.087-7.087a5.996 5.996 0 0 0 .876-7.366 78.504 78.504 0 0 1-9.377-22.65 5.996 5.996 0 0 0-5.83-4.596h-10.034a1.15 1.15 0 0 1-1.148-1.148V359.82c0-.638.526-1.177 1.148-1.177h10.034a5.995 5.995 0 0 0 5.829-4.592 78.694 78.694 0 0 1 9.378-22.654 5.996 5.996 0 0 0-.876-7.366l-7.088-7.088c-.441-.441-.432-1.202-.005-1.633l18.778-18.779c.459-.453 1.219-.462 1.662-.021l7.087 7.087a5.993 5.993 0 0 0 7.366.876 78.694 78.694 0 0 1 22.654-9.378 5.997 5.997 0 0 0 4.592-5.829v-10.034c0-.622.539-1.148 1.177-1.148h26.589a1.15 1.15 0 0 1 1.148 1.148v10.034a5.994 5.994 0 0 0 4.573 5.824c8.088 1.975 15.717 5.132 22.674 9.383a5.992 5.992 0 0 0 7.365-.876l7.085-7.086c.437-.437 1.204-.438 1.643 0l18.793 18.793c.29.29.35.614.35.835 0 .211-.058.519-.357.815l-7.059 7.087a5.995 5.995 0 0 0-.876 7.343 80.385 80.385 0 0 1 9.391 22.688 5.994 5.994 0 0 0 5.824 4.573h10.007c.627 0 1.177.55 1.177 1.177v26.589zm-95.032-70.086c-31.315 0-56.792 25.477-56.792 56.792s25.477 56.763 56.792 56.763 56.764-25.464 56.764-56.763-25.464-56.792-56.764-56.792zm0 101.564c-24.704 0-44.802-20.085-44.802-44.772s20.098-44.802 44.802-44.802 44.773 20.098 44.773 44.802-20.085 44.772-44.773 44.772zm26.321-64.376a5.994 5.994 0 0 1 0 8.479l-30.728 30.728a5.996 5.996 0 0 1-4.239 1.756h-.003a5.989 5.989 0 0 1-4.241-1.761l-13.437-13.465a5.995 5.995 0 1 1 8.489-8.469l9.197 9.216 26.483-26.483a5.994 5.994 0 0 1 8.479 0z"
                                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                            </g>
                                                        </svg>
                                                        <span>Employee Onboard</span>
                                                    </Link>
                                                </li>

                                                {/* <!-- <li>
                                                    <a href="overtime.html" class="drop-link" role="button">
                                                        <svg
                                                            xmlns="http://www.w3.org/2000/svg"
                                                            version="1.1"
                                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                                            width="18"
                                                            height="18"
                                                            x="0"
                                                            y="0"
                                                            viewBox="0 0 512.014 512.014"
                                                            style="enable-background: new 0 0 512 512"
                                                            xml:space="preserve"
                                                            class=""
                                                        >
                                                            <g>
                                                                <path
                                                                    d="M477.867 76.8c-.028-23.552-19.114-42.638-42.667-42.667V25.6c0-14.138-11.462-25.6-25.6-25.6S384 11.462 384 25.6v8.533h-34.133V25.6c0-14.138-11.462-25.6-25.6-25.6s-25.6 11.462-25.6 25.6v8.533h-34.133V25.6c0-14.138-11.462-25.6-25.6-25.6s-25.6 11.462-25.6 25.6v8.533H179.2V25.6c0-14.138-11.462-25.6-25.6-25.6S128 11.462 128 25.6v8.533H93.867V25.6c0-14.138-11.462-25.6-25.6-25.6s-25.6 11.462-25.6 25.6v8.533C19.114 34.162.028 53.248 0 76.8v358.4a42.625 42.625 0 0 0 42.666 42.666h185.353c71.183 51.699 169.392 43.962 231.601-18.247s69.946-160.418 18.247-231.601zm-76.8-51.2a8.533 8.533 0 0 1 17.066 0v34.133a8.533 8.533 0 0 1-17.066 0zm-85.334 0a8.533 8.533 0 0 1 17.066 0v34.133a8.533 8.533 0 0 1-17.066 0zm-85.333 0a8.533 8.533 0 0 1 17.066 0v34.133a8.533 8.533 0 0 1-17.066 0zm-85.333 0a8.533 8.533 0 0 1 17.066 0v34.133a8.533 8.533 0 0 1-17.066 0zm-85.334 0a8.534 8.534 0 0 1 17.067 0v34.133a8.533 8.533 0 0 1-17.066 0V25.6zM42.667 51.2v8.533c0 14.138 11.462 25.6 25.6 25.6s25.6-11.462 25.6-25.6V51.2H128v8.533c0 14.138 11.462 25.6 25.6 25.6s25.6-11.462 25.6-25.6V51.2h34.133v8.533c0 14.138 11.462 25.6 25.6 25.6s25.6-11.462 25.6-25.6V51.2h34.133v8.533c0 14.138 11.462 25.6 25.6 25.6s25.6-11.462 25.6-25.6V51.2H384v8.533c0 14.138 11.462 25.6 25.6 25.6s25.6-11.462 25.6-25.6V51.2c14.138 0 25.6 11.462 25.6 25.6v25.6H17.067V76.8c0-14.138 11.461-25.6 25.6-25.6zM332.8 494.933c-66.512.022-126.286-40.593-150.762-102.437s-8.685-132.365 39.828-177.866v7.236a8.533 8.533 0 0 0 17.066 0v-25.6a8.533 8.533 0 0 0-8.533-8.533h-25.6a8.533 8.533 0 0 0 0 17.066h2.688a178.807 178.807 0 0 0 .136 256H42.667a25.546 25.546 0 0 1-25.6-25.6V119.466H460.8v88.158a178.594 178.594 0 0 0-128-54.025 8.533 8.533 0 0 0 0 17.066c89.544 0 162.133 72.59 162.133 162.133S422.344 494.933 332.8 494.933z"
                                                                    fill="#000000"
                                                                    opacity="1"
                                                                    data-original="#000000"
                                                                    class=""
                                                                ></path>
                                                                <path
                                                                    d="M332.8 443.733a8.533 8.533 0 0 0-8.533 8.533v17.067a8.533 8.533 0 0 0 17.066 0v-17.067a8.533 8.533 0 0 0-8.533-8.533zM399.923 431.992a8.534 8.534 0 0 0-14.817-.033 8.533 8.533 0 0 0 .038 8.566l8.533 14.78a8.534 8.534 0 0 0 14.817.033 8.533 8.533 0 0 0-.038-8.566zM455.305 393.677l-14.771-8.533a8.533 8.533 0 1 0-8.533 14.779l14.771 8.533a8.533 8.533 0 1 0 8.533-14.779zM443.733 332.8a8.533 8.533 0 0 0 8.533 8.533h17.067a8.533 8.533 0 0 0 0-17.066h-17.067a8.533 8.533 0 0 0-8.533 8.533zM432 265.677a8.533 8.533 0 1 0 8.533 14.779l14.771-8.533a8.533 8.533 0 1 0-8.533-14.779zM405.333 207.164a8.534 8.534 0 0 0-11.657 3.132l-8.533 14.78a8.533 8.533 0 1 0 14.779 8.533l8.533-14.78c1.134-1.961 1.442-4.292.857-6.48s-2.016-4.054-3.979-5.185zM332.8 221.867a8.533 8.533 0 0 0 8.533-8.533v-17.067a8.533 8.533 0 0 0-17.066 0v17.067a8.533 8.533 0 0 0 8.533 8.533zM271.923 210.295a8.533 8.533 0 1 0-14.779 8.533l8.533 14.78a8.532 8.532 0 0 0 11.628 3.074 8.533 8.533 0 0 0 3.151-11.608zM233.6 265.677l-14.771-8.533a8.533 8.533 0 1 0-8.533 14.779l14.771 8.533a8.533 8.533 0 1 0 8.533-14.779zM221.867 332.8a8.533 8.533 0 0 0-8.533-8.533h-17.067a8.533 8.533 0 0 0 0 17.066h17.067a8.533 8.533 0 0 0 8.533-8.533zM225.067 385.143l-14.771 8.533a8.534 8.534 0 0 0-.033 14.817 8.533 8.533 0 0 0 8.566-.038l14.771-8.533a8.534 8.534 0 0 0 .033-14.817 8.536 8.536 0 0 0-8.566.038zM277.333 428.868a8.534 8.534 0 0 0-11.657 3.123l-8.533 14.78a8.533 8.533 0 1 0 14.779 8.533l8.533-14.78a8.53 8.53 0 0 0-3.122-11.656zM392.533 341.333a8.533 8.533 0 0 0 0-17.066H356.83a25.602 25.602 0 0 0-15.497-15.497v-61.303a8.533 8.533 0 0 0-17.066 0v61.303c-11.234 3.948-18.249 15.135-16.911 26.967s10.676 21.169 22.508 22.508c11.832 1.338 23.019-5.677 26.967-16.911h35.702zm-59.733 0a8.533 8.533 0 1 1 0-17.066 8.533 8.533 0 0 1 0 17.066z"
                                                                    fill="#000000"
                                                                    opacity="1"
                                                                    data-original="#000000"
                                                                    class=""
                                                                ></path>
                                                            </g>
                                                        </svg>
                                                        <span>Overtime</span>
                                                    </a>
                                                </li> --> */}
                                                <li>
                                                    <Link to={RoutesLink?.leave_configuration_route} onboard
                                                        onClick={() => toggleIsActive('leave-setting')}
                                                        className={isActive === 'leave-setting' ? "drop-link active" : "drop-link"}
                                                        role="button">
                                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                                            width="18" height="18" x="0" y="0"
                                                            viewBox="0 0 1706.66 1706.66" style={{ enableBackground: "new 0 0 512 512" }}
                                                            fill-rule="evenodd">
                                                            <g>
                                                                <path
                                                                    d="M1395.65 916.5h-220.77c-14.73 0-26.67-11.93-26.67-26.67V106.27c0-14.74 11.93-26.67 26.67-26.67h220.77c14.74 0 26.67 11.93 26.67 26.67v783.57c0 14.72-11.94 26.66-26.67 26.66zm-194.11-53.33h167.44V132.94h-167.44z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                                <path
                                                                    d="M1395.65 996.09c-14.68 0-26.67-11.89-26.67-26.67V26.69c0-18.03 17.52-30.82 34.66-25.44l253.43 79.58a26.657 26.657 0 0 1 18.69 25.44v783.58c0 11.65-7.56 21.96-18.69 25.44l-261.42 80.79zm26.67-933.08V933.1l200.09-62.82V125.85z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                                <path
                                                                    d="M1476.49 572.93c-14.74 0-26.67-11.93-26.67-26.67v-91.7c0-35.11 53.33-35.11 53.33 0v91.7c0 14.73-11.93 26.67-26.67 26.67zM382.59 1362.79c-133.75 0-242.56-108.81-242.56-242.56 0-133.74 108.8-242.56 242.56-242.56s242.56 108.81 242.56 242.56-108.8 242.56-242.56 242.56zm0-431.8c-104.35 0-189.23 84.89-189.23 189.23 0 104.35 84.89 189.23 189.23 189.23s189.23-84.88 189.23-189.23c0-104.34-84.89-189.23-189.23-189.23z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                                <path
                                                                    d="M707.61 1646.02H57.57c-14.74 0-26.67-11.93-26.67-26.67v-76.62c0-124.35 67.16-240.8 175.3-303.94 11.06-6.47 25.15-4.14 33.57 5.53 75.52 86.8 210.28 86.7 285.67 0 8.41-9.67 22.51-12 33.57-5.53 108.13 63.14 175.28 179.61 175.28 303.94v76.62c0 14.74-11.95 26.67-26.67 26.67zm-623.38-53.33h596.71v-49.96c0-98.53-49.79-191.26-131.2-246.75-93.6 88.98-240.69 89.04-334.27 0-81.43 55.47-131.23 148.21-131.23 246.75v49.96zM978.5 1706.64c-25.16 0-45.63-20.5-45.63-45.7l-.02-394.96c0-25.2 20.52-45.69 45.7-45.69h600.85c25.2 0 45.71 20.52 45.71 45.7v411.19c0 14.69-11.87 26.6-26.54 26.67l-620.08 2.79zm7.68-433.01.02 379.65 585.58-2.61v-377.02h-585.6z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                                <path
                                                                    d="M1320.2 1513.13c-34.01 0-35.69-51.26-1.56-53.29 107.65-6.42 205.7-39.68 262.3-88.97 26.47-23.08 61.48 17.16 35.01 40.21-66.13 57.63-173.35 94.8-294.15 102.02-.54.01-1.06.02-1.61.02zm-82.43 0c-.54 0-1.07-.02-1.61-.04-120.78-7.22-227.98-44.39-294.15-102.02-26.5-23.07 8.56-63.29 35.02-40.24 56.61 49.31 154.67 82.57 262.3 88.99 34.13 2.04 32.44 53.3-1.56 53.3zM1424.34 1273.63h-290.72c-35.42 0-26.67-47-26.67-85.98 0-47.4 38.57-85.97 85.97-85.97h172.09c47.4 0 85.97 38.57 85.97 85.97 0 38.99 8.75 85.98-26.65 85.98zm-264.04-53.33h237.38c0-22.85 5.8-65.3-32.65-65.3h-172.09c-38.44 0-32.65 42.44-32.66 65.3z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                                <path
                                                                    d="M1278.98 1546.77c-90.51 0-90.5-137.51 0-137.51 90.46 0 90.51 137.51 0 137.51zm0-84.19c-20.3 0-20.33 30.85 0 30.85 20.3 0 20.28-30.85 0-30.85zM698.76 394.43c-23.92 0-35.9-29.44-18.33-46.04l62.6-59.19-62.6-59.21c-25.52-24.13 11.15-62.85 36.64-38.75l83.07 78.58c11.11 10.52 11.13 28.23 0 38.75l-83.07 78.57c-5.14 4.9-11.73 7.3-18.31 7.3z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                                <path
                                                                    d="M776.63 310.93H473.16c-35.11 0-35.11-53.33 0-53.33h303.47c35.11 0 35.11 53.33 0 53.33z"
                                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                                            </g>
                                                        </svg>
                                                        <span>Leave Setting</span>
                                                    </Link>
                                                </li>
                                            </ul>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <Link to={RoutesLink?.switch_user} className="drop-link" role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 24 24" style={{ enableBackground: " new 0 0 512 512" }}
                                            className="">
                                            <g>
                                                <g fill="#000" fill-rule="evenodd" clip-rule="evenodd">
                                                    <path
                                                        d="M6.5 2.75c-.821 0-1.5.675-1.5 1.523 0 .848.679 1.522 1.5 1.522S8 5.121 8 4.273 7.321 2.75 6.5 2.75zm-3 1.523c0-1.663 1.336-3.023 3-3.023s3 1.36 3 3.023-1.336 3.022-3 3.022-3-1.36-3-3.022zm5.354 5.424c-1.436-.778-3.272-.778-4.708 0-.1.053-.202.107-.306.16-.35.181-.707.366-.958.589a.956.956 0 0 0-.102.103.866.866 0 0 0 .067.064c.478.414.924.637 1.448.637h4.41c.524 0 .97-.223 1.448-.637a.88.88 0 0 0 .067-.064.96.96 0 0 0-.102-.103c-.25-.223-.608-.408-.958-.589a18.27 18.27 0 0 1-.306-.16zm1.406.799-.003.008c.001-.006.003-.008.003-.008zm-7.52 0 .004.008-.004-.008zm.692-2.118c1.881-1.019 4.255-1.019 6.136 0l.183.094c.35.18.953.487 1.364.853.252.224.567.581.626 1.072.066.55-.211 1.008-.606 1.35-.627.543-1.41 1.003-2.43 1.003h-4.41c-1.02 0-1.803-.46-2.43-1.003-.395-.342-.672-.8-.605-1.35.058-.49.373-.848.625-1.072.411-.366 1.014-.674 1.364-.853a9.69 9.69 0 0 0 .183-.094zM4 14.25a.75.75 0 0 1 .75.75 5.247 5.247 0 0 0 3.956 5.09l-.234-.469a.75.75 0 1 1 1.342-.67l.857 1.714A.75.75 0 0 1 10 21.75 6.746 6.746 0 0 1 3.25 15a.75.75 0 0 1 .75-.75zM13.362 2.606A.75.75 0 0 1 14 2.25 6.746 6.746 0 0 1 20.75 9a.75.75 0 1 1-1.5 0 5.247 5.247 0 0 0-3.956-5.09l.234.469a.75.75 0 0 1-1.342.67l-.857-1.714a.75.75 0 0 1 .033-.73zM17.5 12.75c-.821 0-1.5.675-1.5 1.523s.679 1.523 1.5 1.523 1.5-.675 1.5-1.523-.679-1.523-1.5-1.523zm-3 1.523c0-1.663 1.336-3.023 3-3.023s3 1.36 3 3.023-1.336 3.023-3 3.023-3-1.361-3-3.023zm5.354 5.424c-1.436-.778-3.272-.778-4.708 0l-.306.16c-.35.181-.707.366-.958.589a.96.96 0 0 0-.102.103.881.881 0 0 0 .067.064c.478.414.924.637 1.448.637h4.41c.524 0 .97-.223 1.448-.637a.9.9 0 0 0 .067-.064.96.96 0 0 0-.102-.103c-.25-.223-.608-.408-.958-.589a17.952 17.952 0 0 1-.306-.16zm1.406.799-.004.008.004-.008zm-7.52 0 .004.008a.036.036 0 0 1-.004-.008zm.692-2.118c1.881-1.019 4.255-1.019 6.136 0l.183.094c.35.18.953.487 1.364.853.252.224.567.581.626 1.072.066.55-.211 1.008-.606 1.35-.627.543-1.41 1.003-2.43 1.003h-4.41c-1.02 0-1.803-.46-2.43-1.003-.395-.342-.672-.8-.606-1.35.06-.49.374-.848.626-1.072.411-.366 1.014-.674 1.364-.853.073-.037.136-.069.183-.095z"
                                                        fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                </g>
                                            </g>
                                        </svg>
                                        <span>Switch User</span>
                                    </Link>
                                </li>
                            </ul>
                        </nav>
                    </div>}
                    {role === 'user' && <div className="sidebar-inner">
                        <nav className="aside-nav">
                            <ul className="sidebar-vertical">
                                <li>
                                    <span className="menu-title"> Menu </span>
                                </li>
                                <li>
                                    <Link to={RoutesLink?.user_dashboard}
                                        onClick={() => toggleIsActive('dashboard')}
                                        className={isActive === 'dashboard' ? 'drop-link active' : 'drop-link'}
                                        role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 463.937 463.937"
                                            className="svg-custom">
                                            <g>
                                                <path
                                                    d="m460.414 216.518-85.6-73.1c.1-.5.2-1 .3-1.6v-95.6c-.1-5.7-4.6-10.3-10.3-10.5h-47.8c-5.5 0-10 4.9-10 10.5v39.6l-68.5-58.4c-3.7-3.2-9.2-3.2-13 0l-221.9 189c-4.3 3.5-4.8 9.8-1.3 14.1s9.8 4.8 14.1 1.3l.2-.2 215.4-183.4 77.1 65.7 46.1 39.2 92.3 78.6c4.2 3.6 10.5 3.1 14.1-1.1 3.5-4.2 3-10.5-1.2-14.1zm-105.4-90-28-23.6v-47.1h28v70.7z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="m416.314 236.718-28.1-24-149.7-127.9c-3.7-3.2-9.2-3.2-13 0l-149.7 127.9-28.1 24c-4.2 3.6-4.7 9.9-1.1 14.1 3.5 4.2 9.7 4.7 13.8 1.2l.1-.1 12.5-10.8v187.5c.1 5.6 4.7 10.2 10.3 10.3h297.3c5.6-.1 10.2-4.6 10.3-10.3v-187.5l12.5 10.8c1.8 1.5 4 2.4 6.4 2.4 2.9 0 5.7-1.3 7.6-3.5 3.7-4.3 3.1-10.6-1.1-14.1zm-144.3 182.1h-80v-108h80v108zm20 0v-117.7c0-5.5-4.1-10.3-9.6-10.3h-100.8c-5.5 0-9.6 4.8-9.6 10.3v117.7h-79v-194.8l139-118.4 139 118.4v194.8h-79z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                            </g>
                                        </svg>
                                        <span className="custom-style"> Dashboard </span>
                                    </Link>
                                </li>
                                <li>
                                    <Link className="drop-link btn-menu" role="button" onClick={toggleAttendance}>
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 66 66" style={{ enableBackground: "new 0 0 512 512" }}
                                            class="">
                                            <g>
                                                <path
                                                    d="M62.5.7h-59c-1.6 0-3 1.3-3 3v51.6c0 1.6 1.3 3 3 3h6.7v6c0 .6.4 1 1 1s1-.4 1-1V56h13.7v8.3c0 .6.4 1 1 1s1-.4 1-1v-6h34.6c1.6 0 3-1.3 3-3V3.7c0-1.6-1.3-3-3-3zM12.2 54v-1.8c0-.4-.2-.7-.5-.9l-3.9-2.2V44l2.4-1.4V45c0 .6.4 1 1 1s1-.4 1-1V26.5c0-.4.4-1 1-1 .5 0 1 .4 1 1V39.4c0 .6.4 1 1 1 .3 0 .5-.1.7-.3s.3-.4.3-.7v-4.5c0-.5.4-1 1-1 .5 0 1 .4 1 1v4.5c0 .6.4 1 1 1s1-.4 1-1v-4.5c0-.5.4-1 1-1 .5 0 1 .4 1 1v4.5c0 .6.4 1 1 1s1-.4 1-1v-4.5c0-.5.4-1 1-1 .5 0 1 .4 1 1V54zM7.6 7.7h11.2v4H7.6zm0 6h11.2v14.6h-2.6v-1.8c0-1.7-1.4-3-3-3-1.7 0-3 1.5-3 3v1.8H7.6zm55.9 41.6c0 .5-.4 1-1 1H27.9V34.9c0-1.6-1.3-3-3-3-.8 0-1.4.3-2 .8-.5-.5-1.2-.8-2-.8s-1.4.3-2 .8c-.5-.5-1.2-.8-2-.8-.3 0-.7.1-1 .2v-1.8h3.6c.6 0 1-.4 1-1V6.7c0-.6-.4-1-1-1H6.6c-.4 0-.8.3-.9.6-.1.2-.1.3-.1.4v22.5c0 .6.4 1 1 1h3.6v9.9l-3.9 2.4c-.3.2-.5.5-.5.9v6.2c0 .4.2.7.5.9l3.9 2.2v3.6H3.5c-.5 0-1-.4-1-1V3.7c0-.5.4-1 1-1h59c.5 0 1 .4 1 1z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M59.5 5.7H30.7c-.6 0-1 .4-1 1v14.2c0 .6.4 1 1 1h28.8c.6 0 1-.4 1-1V6.7c0-.5-.4-1-1-1zm-1 14.2H31.7V7.7h26.8zM38.3 28.3h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1s-.5-1-1-1zM57.3 28.3h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1s-.4-1-1-1zM47.8 28.3h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1s-.4-1-1-1zM38.3 38.7h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.5-1-1-1zM57.3 38.7h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1zM47.8 38.7h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1zM38.3 49.2h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.5-1-1-1zM57.3 49.2h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1zM47.8 49.2h-5.4c-.6 0-1 .4-1 1s.4 1 1 1h5.4c.6 0 1-.4 1-1 0-.5-.4-1-1-1z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M34.1 17.2h4.8c.6 0 1-.4 1-1v-4.8c0-.6-.4-1-1-1h-4.8c-.6 0-1 .4-1 1v4.8c0 .5.4 1 1 1zm1-4.8h2.8v2.8h-2.8zM42.7 17.2h4.8c.6 0 1-.4 1-1v-4.8c0-.6-.4-1-1-1h-4.8c-.6 0-1 .4-1 1v4.8c0 .5.5 1 1 1zm1-4.8h2.8v2.8h-2.8zM51.4 17.2h4.8c.6 0 1-.4 1-1v-4.8c0-.6-.4-1-1-1h-4.8c-.6 0-1 .4-1 1v4.8c0 .5.4 1 1 1zm1-4.8h2.8v2.8h-2.8z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                            </g>
                                        </svg>
                                        <span>Attendance</span>
                                        <svg className={attendance ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 20 20" fill="none">
                                            <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round"></path>
                                        </svg>
                                    </Link>
                                    <ul className="sub-menu inner" style={{ display: attendance ? "block" : 'none' }}>
                                        <li>
                                            <Link to={RoutesLink?.user_attendance}
                                                onClick={() => toggleIsActive('user-attendance')}
                                                className={isActive === 'user-attendance' ? "drop-link inner active" : "drop-link inner "} role="button">
                                                <span>My Attendance</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to={RoutesLink?.user_attendance_list}
                                                onClick={() => toggleIsActive('attendance-list')}
                                                className={isActive === 'attendance-list' ? "drop-link inner active " : "drop-link inner "}
                                                role="button">
                                                <span>Attendance List</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link
                                                to={RoutesLink?.user_outdoor_attendance_route}
                                                onClick={() => toggleIsActive('outdoor')}
                                                className={isActive === 'outdoor' ? "drop-link inner active " : "drop-link inner "}
                                                role="button"
                                            >
                                                <span>OD Request</span>
                                            </Link>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <Link className="drop-link btn-menu" onClick={toggleLeave} role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 32 32" style={{ enableBackground: "new 0 0 512 512" }}
                                            className="">
                                            <g>
                                                <path
                                                    d="M30.5 31h-15a.5.5 0 0 1 0-1H30V2H15a.5.5 0 0 1 0-1h15.5a.5.5 0 0 1 .5.5v29a.5.5 0 0 1-.5.5zM3.785 31a.5.5 0 0 1-.484-.63l2.748-10.188a.5.5 0 1 1 .966.259L4.267 30.63a.499.499 0 0 1-.482.37zM11 7C9.346 7 8 5.654 8 4s1.346-3 3-3 3 1.346 3 3-1.345 3-3 3zm0-5c-1.103 0-2 .897-2 2s.897 2 2 2 2-.897 2-2-.896-2-2-2zM9.046 20.171 5.432 30.336a.5.5 0 0 0 .942.336l3.443-9.683z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M13.568 31.004a.5.5 0 0 1-.49-.403l-1.211-6.153a2.5 2.5 0 0 0-.632-1.223l-3.699-3.928a3.51 3.51 0 0 1-.945-2.64l.36-5.23-2.169.677-.821 3.204c-.169.767-.969 1.294-1.782 1.125a1.505 1.505 0 0 1-1.144-1.79l1.003-3.981a1.5 1.5 0 0 1 .855-1.032l4.505-2.004c.229-.102.481-.145.73-.121l4.44.402a3.5 3.5 0 0 1 2.538 1.458l1.921 2.703 3.035-.362c.38-.064.776.037 1.099.276.322.237.533.586.594.982.061.395-.036.79-.273 1.113s-.587.534-.983.595l-3.75.692a1.487 1.487 0 0 1-1.351-.432l-1.107-1.145a.5.5 0 1 1 .719-.695l1.107 1.145a.495.495 0 0 0 .449.145l3.767-.695c.148-.023.265-.094.344-.201s.111-.239.091-.37a.505.505 0 0 0-.198-.329.51.51 0 0 0-.371-.091l-3.346.4a.486.486 0 0 1-.467-.206l-2.093-2.945a2.493 2.493 0 0 0-1.812-1.042l-4.442-.402a.501.501 0 0 0-.235.039l-4.507 2.004a.498.498 0 0 0-.286.349l-1.003 3.979a.5.5 0 0 0 .384.583.515.515 0 0 0 .596-.38l.895-3.49a.502.502 0 0 1 .335-.354l3.133-.978a.505.505 0 0 1 .459.085c.129.103.2.262.189.427l-.41 5.96a2.506 2.506 0 0 0 .674 1.886l3.7 3.928c.45.478.756 1.07.885 1.713l1.211 6.155a.5.5 0 0 1-.491.597z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M15.583 31.004a.5.5 0 0 1-.498-.465l-.504-7.125a2.5 2.5 0 0 0-.289-1.002l-2.232-4.177a.5.5 0 0 1-.058-.268l.471-7.224a.488.488 0 0 1 .532-.467.5.5 0 0 1 .466.531l-.462 7.082 2.165 4.051c.233.437.37.908.404 1.402l.504 7.125a.5.5 0 0 1-.463.534c-.012.003-.024.003-.036.003zM26.5 6h-9a.5.5 0 0 1 0-1h9a.5.5 0 0 1 0 1z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M24.5 8a.5.5 0 0 1-.354-.853L25.793 5.5l-1.646-1.646a.5.5 0 0 1 .707-.707l2 2a.5.5 0 0 1 0 .707l-2 2A.502.502 0 0 1 24.5 8zM26.5 17c-.827 0-1.5-.673-1.5-1.5s.673-1.5 1.5-1.5 1.5.673 1.5 1.5-.673 1.5-1.5 1.5zm0-2a.5.5 0 1 0 .002 1.002A.5.5 0 0 0 26.5 15z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                            </g>
                                        </svg>
                                        <span>Leave</span>
                                        <svg className={leave ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 20 20" fill="none">
                                            <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round"></path>
                                        </svg>
                                    </Link>
                                    <ul className="sub-menu inner" style={{ display: leave ? "block" : 'none' }}>
                                        <li>
                                            <Link to={RoutesLink?.user_leave}
                                                onClick={() => toggleIsActive('user-leave')}
                                                className={isActive === 'user-leave' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <span>My Leaves</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to={RoutesLink?.user_leave_request}
                                                onClick={() => toggleIsActive('leave-requests')}
                                                className={isActive === 'leave-requests' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <span>Leave Requests</span>
                                            </Link>
                                        </li>
                                    </ul>
                                </li>



                                <li>
                                    <Link className="drop-link btn-menu" onClick={toggleRegularization} role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 64 64" style={{ enableBackground: " new 0 0 512 512" }}
                                            className="">
                                            <g>
                                                <path d="m32.859 44.867-1.982.266c.081.613.123 1.241.123 1.867h2c0-.714-.047-1.432-.141-2.133z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M60 7h-1V4c0-1.654-1.346-3-3-3h-2c-1.654 0-3 1.346-3 3v3H23V4c0-1.654-1.346-3-3-3h-2c-1.654 0-3 1.346-3 3v3h-1c-1.654 0-3 1.346-3 3v22.178C5.144 34.557 1 40.3 1 47c0 8.822 7.178 16 16 16 6.619 0 12.423-4.099 14.821-10H60c1.654 0 3-1.346 3-3V10c0-1.654-1.346-3-3-3zm-7-3c0-.551.449-1 1-1h2c.551 0 1 .449 1 1v8c0 .551-.449 1-1 1h-2c-.551 0-1-.449-1-1zM17 4c0-.551.449-1 1-1h2c.551 0 1 .449 1 1v8c0 .551-.449 1-1 1h-2c-.551 0-1-.449-1-1zm13.876 44.868C29.955 55.784 23.99 61 17 61 9.28 61 3 54.72 3 47s6.28-14 14-14c6.279 0 11.829 4.222 13.497 10.266l1.928-.531C30.518 35.825 24.175 31 17 31c-1.383 0-2.719.195-4 .527V10c0-.551.449-1 1-1h1v3c0 1.654 1.346 3 3 3h2c1.654 0 3-1.346 3-3V9h28v3c0 1.654 1.346 3 3 3h2c1.654 0 3-1.346 3-3V9h1c.551 0 1 .449 1 1v9H19v2h42v19.879l-3.186-4.46a1.002 1.002 0 0 0-.687-.411 1.029 1.029 0 0 0-.768.224l-3.36 2.8V35h6v-2h-7a1 1 0 0 0-1 1v6.698L38.638 51h-6.157c.158-.611.293-1.23.378-1.868zm25.936-10.41 3.662 5.126L43.787 51h-2.025zM60 51H48.713L61 45.539V50c0 .552-.449 1-1 1z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                                <path
                                                    d="M15 19h2v2h-2zM23 30v-6a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1zm-2-1h-4v-4h4zM34 31a1 1 0 0 0 1-1v-6a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1zm-5-6h4v4h-4zM40 31h6a1 1 0 0 0 1-1v-6a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1zm1-6h4v4h-4zM52 31h6a1 1 0 0 0 1-1v-6a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1zm1-6h4v4h-4zM33 41h2v-7a1 1 0 0 0-1-1h-6v2h5zM40 41h6a1 1 0 0 0 1-1v-6a1 1 0 0 0-1-1h-6a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1zm1-6h4v4h-4zM24.707 38.293a.999.999 0 0 0-1.414 0L13 48.586l-3.293-3.293a.999.999 0 0 0-1.414 0l-3 3a.999.999 0 0 0 0 1.414l7 7a.997.997 0 0 0 1.414 0l14-14a.999.999 0 0 0 0-1.414zM13 54.586 7.414 49 9 47.414l3.293 3.293a.999.999 0 0 0 1.414 0L24 40.414 25.586 42z"
                                                    fill="#000000" opacity="1" data-original="#000000" className=""></path>
                                            </g>
                                        </svg>
                                        <span>Att. Regularization</span>
                                        <svg className={regularization ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 20 20" fill="none">
                                            <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round"></path>
                                        </svg>
                                    </Link>
                                    <ul className="sub-menu inner" style={{ display: regularization ? "block" : 'none' }}>
                                        <li>
                                            <Link to={RoutesLink?.user_attendance_regularization}
                                                onClick={() => toggleIsActive('regularization')}
                                                className={isActive === 'regularization' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <span>My A.R.</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to={RoutesLink?.user_attendance_regularization_request}
                                                onClick={() => toggleIsActive('regularization-req')}
                                                className={isActive === 'regularization-req' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <span>A.R. Requests</span>
                                            </Link>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <Link className="drop-link btn-menu" onClick={toggleReimbursement} role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 64 64" style={{ enableBackground: " new 0 0 512 512" }}
                                            className="">
                                            <g>
                                                <path
                                                    d="M56.716 44.31c0-.513-.098-1.016-.289-1.485a3.974 3.974 0 0 0-4.675-2.393c-.045.012-.09.026-.133.044L38.376 45.93c-.015-.106-.02-.213-.047-.318a3.148 3.148 0 0 0-3.445-2.366c-.045.006-.09.015-.134.026l-13.84 3.706-4.929-.824V43a1 1 0 0 0-1-1H10a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1h4.981a1 1 0 0 0 1-1v-2h6.788l6.175 2.325c.556.21 1.14.315 1.729.315.786 0 1.579-.188 2.325-.568l21.51-13.177a3.974 3.974 0 0 0 2.208-3.585zM13.981 60H11V44h2.981zM53.54 46.147 32.022 59.328c-.734.372-1.6.417-2.373.126l-6.346-2.39a.988.988 0 0 0-.352-.064h-6.97v-8.819l4.812.805c.14.023.286.017.424-.021l13.975-3.742a1.153 1.153 0 0 1 1.058 1.763c-.165.267-.423.45-.727.519a.986.986 0 0 0-.128.038l-6.813 2.52.693 1.875 6.756-2.498a3.099 3.099 0 0 0 1.666-1.067l14.612-6.018c.94-.205 1.901.3 2.26 1.211.097.24.146.49.146.744.001.77-.42 1.458-1.175 1.837zM32 26c-.551 0-1-.448-1-1h-2c0 1.302.839 2.402 2 2.816V29h2v-1.184A2.996 2.996 0 0 0 35 25c0-1.654-1.346-3-3-3a1.001 1.001 0 0 1 0-2c.551 0 1 .448 1 1h2a2.996 2.996 0 0 0-2-2.816V17h-2v1.184A2.996 2.996 0 0 0 29 21c0 1.654 1.346 3 3 3a1.001 1.001 0 0 1 0 2z"
                                                    fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                <path
                                                    d="M19 13a1 1 0 0 0-1 1v18a1 1 0 0 0 1 1h26a1 1 0 0 0 1-1V14a1 1 0 0 0-1-1zm25 18H20V15h24z"
                                                    fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                <path
                                                    d="M8.006 34.891c-.042.389.145.766.479.967l5 3a1.007 1.007 0 0 0 .92.056 1 1 0 0 0 .576-.718l1-5a.998.998 0 0 0-1.297-1.144l-2.193.731A20.825 20.825 0 0 1 11 25c0-11.579 9.42-21 21-21s21 9.421 21 21c0 4.501-1.403 8.792-4.056 12.408l1.612 1.184A22.824 22.824 0 0 0 55 25C55 12.317 44.682 2 32 2S9 12.317 9 25c0 2.916.535 5.741 1.589 8.417l-1.906.635a.998.998 0 0 0-.677.839zm5.676-.398-.384 1.919-1.919-1.151z"
                                                    fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                            </g>
                                        </svg>
                                        <span>Reimbursement</span>
                                        <svg className={reimbursement ? "chevron ms-auto dropdown rotate" : "chevron ms-auto dropdown"} xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                            viewBox="0 0 20 20" fill="none">
                                            <path d="M6.4001 8.20005L10.0001 11.8L13.6001 8.20005" stroke="#818898" stroke-width="1.5"
                                                stroke-linecap="round" stroke-linejoin="round"></path>
                                        </svg>
                                    </Link>
                                    <ul className="sub-menu inner" style={{ display: reimbursement ? "block" : 'none' }}>
                                        <li>
                                            <Link to={RoutesLink?.user_reimbursement}
                                                onClick={() => toggleIsActive('reimbursement')}
                                                className={isActive === 'reimbursement' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <span>My Reimbursement</span>
                                            </Link>
                                        </li>
                                        <li>
                                            <Link to={RoutesLink?.user_reimbursement_request}
                                                onClick={() => toggleIsActive('reimbursement-req')}
                                                className={isActive === 'reimbursement-req' ? "drop-link inner active" : "drop-link inner"}
                                                role="button">
                                                <span>Reimbursement Req.</span>
                                            </Link>
                                        </li>
                                    </ul>
                                </li>
                                <li>
                                    <Link to={RoutesLink?.user_holiday_route}
                                        onClick={() => toggleIsActive('user-holidays')}
                                        className={isActive === 'user-holidays' ? "drop-link active" : "drop-link"} role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 60 60" style={{ enableBackground: " new 0 0 512 512" }}
                                            className="">
                                            <g>
                                                <path
                                                    d="M55 4h-2V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1h-4V3a3 3 0 0 0-6 0v1H5a5.006 5.006 0 0 0-5 5v38a5.006 5.006 0 0 0 5 5h2.01a13.871 13.871 0 0 0-1.91 5.8 2.029 2.029 0 0 0 .52 1.547A2 2 0 0 0 7.1 60H53a2 2 0 0 0 1.477-.651 2.03 2.03 0 0 0 .52-1.548 13.881 13.881 0 0 0-1.91-5.8H55a5.006 5.006 0 0 0 5-5V9a5.006 5.006 0 0 0-5-5Zm-6-1a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM39 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM29 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM19 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM9 3a1 1 0 0 1 2 0v4a1 1 0 0 1-2 0ZM5 6h2v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h4v1a3 3 0 0 0 6 0V6h2a3 3 0 0 1 3 3v3H2V9a3 3 0 0 1 3-3Zm2.1 52-.005-.018c.68-7.493 8.781-13.524 19.115-14.743l1.117 2.573a1 1 0 0 0 .918.6.987.987 0 0 0 .4-.083 1 1 0 0 0 .518-1.315l-.849-1.956c.575-.03 1.151-.059 1.737-.059a30.559 30.559 0 0 1 12.33 2.505L41.48 47h-8.9a2 2 0 0 0-1.664.891l-1.7 2.554a1 1 0 1 0 1.664 1.11L32.581 49h8.93l1.7 2.555a1 1 0 1 0 1.664-1.11l-1.649-2.473.949-1.582C49.2 49.119 52.569 53.269 53 58ZM55 50h-3.312a21.742 21.742 0 0 0-6.494-5.3l.709-1.182a1 1 0 0 0-1.715-1.03L43.4 43.8A32.307 32.307 0 0 0 30.046 41c-.874 0-1.737.035-2.589.1L24 33.129a10.824 10.824 0 0 1 3.6-.633.962.962 0 0 0 .878-.432 7.518 7.518 0 0 1 6.36-2.764 1.008 1.008 0 0 0 1.018-.5 1 1 0 0 0-.1-1.128c-3.787-4.6-10.645-5.979-16.679-3.358s-9.7 8.576-8.932 14.482a1 1 0 0 0 .757.843 1.046 1.046 0 0 0 .235.027 1 1 0 0 0 .824-.434 7.522 7.522 0 0 1 6.365-2.764 1.01 1.01 0 0 0 .913-.345 10.927 10.927 0 0 1 2.921-2.2l3.211 7.393C18.159 42.309 12 45.546 8.4 50H5a3 3 0 0 1-3-3V14h56v33a3 3 0 0 1-3 3ZM18.878 33.727c-1.039-3.15-.675-5.758 1.033-7.493 2.414-.061 4.584 1.451 6.18 4.36a14.313 14.313 0 0 0-7.213 3.133Zm9.029-3.972a11.084 11.084 0 0 0-4.32-4.633 12.065 12.065 0 0 1 9.071 2.3 9.608 9.608 0 0 0-4.751 2.331ZM17.026 34.48a9.62 9.62 0 0 0-4.945 1.881 12.059 12.059 0 0 1 4.5-8.194 11.081 11.081 0 0 0 .445 6.313Z"
                                                    fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                <path
                                                    d="M50.046 24H48.9a3.915 3.915 0 0 0-.425-1.019l.809-.809a1 1 0 1 0-1.414-1.414l-.809.809a3.915 3.915 0 0 0-1.019-.425V20a1 1 0 1 0-2 0v1.142a3.915 3.915 0 0 0-1.019.425l-.809-.809a1 1 0 0 0-1.414 1.414l.809.809A3.915 3.915 0 0 0 41.188 24h-1.142a1 1 0 1 0 0 2h1.142a3.915 3.915 0 0 0 .425 1.019l-.809.809a1 1 0 1 0 1.414 1.414l.809-.809a3.915 3.915 0 0 0 1.019.425V30a1 1 0 0 0 2 0v-1.142a3.915 3.915 0 0 0 1.019-.425l.809.809a1 1 0 1 0 1.414-1.414l-.809-.809A3.915 3.915 0 0 0 48.9 26h1.142a1 1 0 0 0 0-2Zm-5 3a2 2 0 1 1 2-2 2 2 0 0 1-2 2Z"
                                                    fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                            </g>
                                        </svg>
                                        <span>Holidays</span>
                                    </Link>
                                </li>
                                <li>
                                    <Link to={RoutesLink?.user_payslip_route}
                                        onClick={() => toggleIsActive('user-pay-slip')}
                                        className={isActive === 'user-pay-slip' ? "drop-link active" : "drop-link"}
                                        role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 48 48" style={{ enableBackground: " new 0 0 512 512" }}
                                        >
                                            <g>
                                                <path
                                                    d="M41.179 11.116h-38.4A2.782 2.782 0 0 0 0 13.895v24.252a2.782 2.782 0 0 0 2.779 2.78h38.4a2.782 2.782 0 0 0 2.779-2.78V13.895a2.782 2.782 0 0 0-2.78-2.78zm1.263 27.031c0 .697-.566 1.264-1.263 1.264h-38.4a1.264 1.264 0 0 1-1.263-1.264V13.895c0-.697.566-1.263 1.263-1.263h38.4c.697 0 1.263.566 1.263 1.263v24.252zM48 9.853v24.252c0 .998-.518 1.896-1.387 2.403a.757.757 0 1 1-.764-1.309c.398-.232.635-.64.635-1.094V9.853c0-.697-.566-1.264-1.263-1.264h-38.4c-.453 0-.861.238-1.093.635a.759.759 0 0 1-1.31-.764 2.758 2.758 0 0 1 2.403-1.386h38.4A2.782 2.782 0 0 1 48 9.853zM4.8 23.747h9.095c.418 0 .758-.34.758-.757v-7.074a.758.758 0 0 0-.758-.758H4.8a.758.758 0 0 0-.758.758v7.074c0 .418.34.757.758.757zm5.305-1.515V20.21h3.032v2.02zm3.032-3.537h-3.032v-2.021h3.032zm-7.58-2.021H8.59v5.558H5.558zm33.6-1.516H25.012a.758.758 0 0 0-.758.758v4.042c0 .418.34.758.758.758h14.147c.418 0 .758-.34.758-.758v-4.042a.758.758 0 0 0-.758-.758zM38.4 19.2H25.768v-2.526H38.4zM4.042 29.053c0-.419.34-.758.758-.758h4.042a.758.758 0 0 1 0 1.516H4.8a.758.758 0 0 1-.758-.758zm13.642 0c0 .418-.34.758-.758.758h-4.042a.758.758 0 0 1 0-1.516h4.042c.419 0 .758.34.758.758zm8.084 0c0 .418-.34.758-.758.758h-4.042a.758.758 0 0 1 0-1.516h4.042c.419 0 .758.34.758.758zm8.085 0c0 .418-.34.758-.758.758h-4.042a.758.758 0 0 1 0-1.516h4.042c.418 0 .758.34.758.758zm-16.169 5.052c0 .419-.34.758-.758.758H4.8a.758.758 0 0 1 0-1.516h12.126c.419 0 .758.34.758.758z"
                                                    fill="#000000" opacity="1" data-original="#000000"></path>
                                            </g>
                                        </svg>
                                        <span>Pay slip</span>
                                    </Link>
                                </li>
                                <li>
                                    <Link to={RoutesLink?.switch_user}
                                        onClick={() => toggleIsActive('switch-user')}
                                        className={isActive === 'switch-user' ? "drop-link active" : "drop-link"}
                                        role="button">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                                            width="18" height="18" x="0" y="0" viewBox="0 0 24 24" style={{ enableBackground: "new 0 0 512 512" }}
                                            className="">
                                            <g>
                                                <g fill="#000" fill-rule="evenodd" clip-rule="evenodd">
                                                    <path
                                                        d="M6.5 2.75c-.821 0-1.5.675-1.5 1.523 0 .848.679 1.522 1.5 1.522S8 5.121 8 4.273 7.321 2.75 6.5 2.75zm-3 1.523c0-1.663 1.336-3.023 3-3.023s3 1.36 3 3.023-1.336 3.022-3 3.022-3-1.36-3-3.022zm5.354 5.424c-1.436-.778-3.272-.778-4.708 0-.1.053-.202.107-.306.16-.35.181-.707.366-.958.589a.956.956 0 0 0-.102.103.866.866 0 0 0 .067.064c.478.414.924.637 1.448.637h4.41c.524 0 .97-.223 1.448-.637a.88.88 0 0 0 .067-.064.96.96 0 0 0-.102-.103c-.25-.223-.608-.408-.958-.589a18.27 18.27 0 0 1-.306-.16zm1.406.799-.003.008c.001-.006.003-.008.003-.008zm-7.52 0 .004.008-.004-.008zm.692-2.118c1.881-1.019 4.255-1.019 6.136 0l.183.094c.35.18.953.487 1.364.853.252.224.567.581.626 1.072.066.55-.211 1.008-.606 1.35-.627.543-1.41 1.003-2.43 1.003h-4.41c-1.02 0-1.803-.46-2.43-1.003-.395-.342-.672-.8-.605-1.35.058-.49.373-.848.625-1.072.411-.366 1.014-.674 1.364-.853a9.69 9.69 0 0 0 .183-.094zM4 14.25a.75.75 0 0 1 .75.75 5.247 5.247 0 0 0 3.956 5.09l-.234-.469a.75.75 0 1 1 1.342-.67l.857 1.714A.75.75 0 0 1 10 21.75 6.746 6.746 0 0 1 3.25 15a.75.75 0 0 1 .75-.75zM13.362 2.606A.75.75 0 0 1 14 2.25 6.746 6.746 0 0 1 20.75 9a.75.75 0 1 1-1.5 0 5.247 5.247 0 0 0-3.956-5.09l.234.469a.75.75 0 0 1-1.342.67l-.857-1.714a.75.75 0 0 1 .033-.73zM17.5 12.75c-.821 0-1.5.675-1.5 1.523s.679 1.523 1.5 1.523 1.5-.675 1.5-1.523-.679-1.523-1.5-1.523zm-3 1.523c0-1.663 1.336-3.023 3-3.023s3 1.36 3 3.023-1.336 3.023-3 3.023-3-1.361-3-3.023zm5.354 5.424c-1.436-.778-3.272-.778-4.708 0l-.306.16c-.35.181-.707.366-.958.589a.96.96 0 0 0-.102.103.881.881 0 0 0 .067.064c.478.414.924.637 1.448.637h4.41c.524 0 .97-.223 1.448-.637a.9.9 0 0 0 .067-.064.96.96 0 0 0-.102-.103c-.25-.223-.608-.408-.958-.589a17.952 17.952 0 0 1-.306-.16zm1.406.799-.004.008.004-.008zm-7.52 0 .004.008a.036.036 0 0 1-.004-.008zm.692-2.118c1.881-1.019 4.255-1.019 6.136 0l.183.094c.35.18.953.487 1.364.853.252.224.567.581.626 1.072.066.55-.211 1.008-.606 1.35-.627.543-1.41 1.003-2.43 1.003h-4.41c-1.02 0-1.803-.46-2.43-1.003-.395-.342-.672-.8-.606-1.35.06-.49.374-.848.626-1.072.411-.366 1.014-.674 1.364-.853.073-.037.136-.069.183-.095z"
                                                        fill="#000000" opacity="1" data-original="#000000" class=""></path>
                                                </g>
                                            </g>
                                        </svg>
                                        <span>Switch User</span>
                                    </Link>
                                </li>
                            </ul>
                        </nav>
                    </div>}
                </div>
            </aside>
        </>
    )
}

// apply this actual correct code
// // src/components/Sidebar.js
// import React from "react";
// import { Link } from "react-router-dom";
// import { RoutesLink } from "../../../ApiHelper/RoutesLink";
// import { DashboardIcon, SitesIcon, ChevronIcon } from "./Icons";

// export default function Sidebar({ role }) {
//   const [isActive, setIsActive] = React.useState("");
//   const [isSitesOpen, setIsSitesOpen] = React.useState(false);

//   const toggleIsActive = (link) => setIsActive(link);
//   const toggleSites = () => setIsSitesOpen(!isSitesOpen);

//   return (
//     <aside className="sidebar-wrapper">
//       <div className="sidebar-content">
//         <Link className="navbar-brand" to="">
//           <img className="main-logo" src="../assets/img/Shreenath-logo.svg" alt="main-logo" />
//         </Link>
//         {role === "admin" && (
//           <div className="sidebar-inner">
//             <nav className="aside-nav">
//               <ul className="sidebar-vertical">
//                 <li>
//                   <span className="menu-title">Menu</span>
//                 </li>
//                 <li>
//                   <Link
//                     to={RoutesLink.dashboard_route}
//                     onClick={() => toggleIsActive("dashboard")}
//                     className={isActive === "dashboard" ? "drop-link active" : "drop-link"}
//                   >
//                     <DashboardIcon />
//                     <span>Dashboard</span>
//                   </Link>
//                 </li>
//                 <li>
//                   <button className="drop-link btn-menu" onClick={toggleSites}>
//                     <span className="menu-title border-0 p-0 m-0">Sites</span>
//                     <ChevronIcon isOpen={isSitesOpen} />
//                   </button>
//                   {isSitesOpen && (
//                     <ul className="sub-menu">
//                       <li>
//                         <Link
//                           to={RoutesLink.sites_route}
//                           onClick={() => toggleIsActive("sites")}
//                           className={isActive === "sites" ? "drop-link active" : "drop-link"}
//                         >
//                           <SitesIcon />
//                           <span>Sites</span>
//                         </Link>
//                       </li>
//                       <li>
//                         <Link
//                           to={RoutesLink.vendors_route}
//                           onClick={() => toggleIsActive("vendor")}
//                           className={isActive === "vendor" ? "drop-link active" : "drop-link"}
//                         >
//                           <SitesIcon />
//                           <span>Vendors</span>
//                         </Link>
//                       </li>
//                     </ul>
//                   )}
//                 </li>
//               </ul>
//             </nav>
//           </div>
//         )}
//       </div>
//     </aside>
//   );
// }
